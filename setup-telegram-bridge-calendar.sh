...
SAFE_MODELS = ['qwen/qwen3.8-27b', 'meta-llama/llama-4-scout-17b-16e-instruct', 'llama-3.1-8b-instant', 'gemma2-9b-it', 'qwen/qwen3-32b']
try:
    ...
    for safe in SAFE_MODELS:
        if safe in ids:
            print(safe)
            exit()
    print('qwen/qwen3.8-27b')
except (json.JSONDecodeError, KeyError, TypeError):
    print('qwen/qwen3.8-27b')
" 2>/dev/null || echo "qwen/qwen3.8-27b")
...
def format_sources(sources) -> str:
    """✅ OpenWebUI 응답의 sources(도구·RAG 출처)를 Telegram 텍스트로 변환.
    OpenWebUI 채팅의 '소스 1' 클릭 시 보이는 내용을 텍스트로 펼쳐 보여준다.
    sources 구조: [{"source": {"name": "..."}, "document": ["...url 포함 텍스트..."]}, ...]
    """
    if not sources:
        return ""
    blocks = []
    for idx, s in enumerate(sources, 1):
        ...
        if isinstance(src, dict):
            name = src.get("name") or src.get("title") or src.get("url") or ""
        elif isinstance(src, str):
            name = src
        # 본문(document): 리스트 또는 문자열
        docs = s.get("document") or s.get("content") or s.get("text") or ""
        ...
        async with aiohttp.ClientSession(timeout=timeout) as http:
            async with http.post(
                ...
                if resp.status == 200:
                    ...
                    # ✅ 출처(sources)가 있으면 본문 뒤에 붙임 (OpenWebUI '소스 N' 내용)
                    try:
                        _src_txt = format_sources(data.get("sources"))
                        if _src_txt:
                            ai_text = ai_text + _src_txt
                    except Exception as _se:
                        logger.debug(f"sources 처리 오류 (무시): {_se}")
                    return ai_text
                else:
...
async def stream_openwebui_chat(
    ...
    last_len    = 0
    collected_sources = []  # ✅ SSE 청크에서 수집한 출처
    ...
    try:
            ...
            async with http.post(
                ...
                async for raw in resp.content:
                    ...
                    try:
                        ...
                        # ✅ 출처(sources)는 별도 청크로 오거나 청크에 실려 옴 → 수집
                        _cs = chunk.get("sources")
                        if _cs:
                            collected_sources = _cs
        ...
        # ✅ 출처(sources)가 있으면 본문 뒤에 붙임 (OpenWebUI '소스 N' 내용)
        try:
            _src_txt = format_sources(collected_sources)
            if _src_txt:
                return full_text + _src_txt
        except Exception as _se:
            logger.debug(f"sources 처리 오류 (무시): {_se}")
...
# ── [KEY-SYNC] browser-agent API 키 인증 자가진단 ──────────────────────
#   브릿지가 들고 있는 키로 browser-agent 의 /browse/multi 를 실제 호출해
#   인증(401)이 통과하는지 확인한다. 401 이면 브릿지의
#   secrets/browser_agent_api_key 가 서버 키와 어긋난 것 →
#   복잡한 작업 요청이 Multi-Agent 로 안 가고 조용히 폴백된다.
if [ "$HEALTH_STATUS" = "running" ] && [ -n "$BA_NETWORK" ] && [ -s "$BASE_DIR/secrets/browser_agent_api_key" ]; then
    print_header "🔑 browser-agent API 키 인증 자가진단"
    # 브릿지 컨테이너 안에서 컨테이너명(browser-agent:8001)으로 호출 — 실제 통신 경로와 동일
    TG_AUTH_CODE=$(docker exec telegram-openwebui-bridge sh -c \
        'curl -s -o /dev/null -w "%{http_code}" --connect-timeout 4 \
          -X POST http://browser-agent:8001/browse/multi \
          -H "Content-Type: application/json" \
          -H "Authorization: Bearer $(cat /app/secrets/browser_agent_api_key 2>/dev/null || echo x)" \
          -d "{\"task\":\"핑\",\"budget_usd\":0.001}"' 2>/dev/null || echo "000")
    case "$TG_AUTH_CODE" in
        401)
            print_warn "browser-agent 인증 실패(401) — 브릿지 키가 서버 키와 다릅니다."
            # 서버 키가 읽히면 자동 재동기화 시도
            _SRV_KEY=$(cat "$HOME/OpenWebUI/browser-agent/secrets/api_key" 2>/dev/null \
                       || sudo cat "$HOME/OpenWebUI/browser-agent/secrets/api_key" 2>/dev/null || true)
            if [ -n "$_SRV_KEY" ]; then
                echo -n "$_SRV_KEY" > "$BASE_DIR/secrets/browser_agent_api_key"
                chmod 600 "$BASE_DIR/secrets/browser_agent_api_key"
                docker compose restart >/dev/null 2>&1 || docker restart telegram-openwebui-bridge >/dev/null 2>&1 || true
                print_ok "서버 키로 자동 재동기화 후 브릿지 재시작 완료 — 다시 시도해 보세요."
            else
                print_warn "서버 키를 읽지 못했습니다. 수동 동기화:"
                print_info "  sudo cp ~/OpenWebUI/browser-agent/secrets/api_key $BASE_DIR/secrets/browser_agent_api_key"
                print_info "  cd $BASE_DIR && docker compose restart"
            fi
            ;;
        000)
            print_warn "browser-agent 응답 없음 — 네트워크(openwebui_net) 연결을 확인하세요."
            ;;
        *)
            print_ok "browser-agent 인증 통과 (HTTP ${TG_AUTH_CODE}) — Multi-Agent 정상 연동"
            ;;
    esac
fi
