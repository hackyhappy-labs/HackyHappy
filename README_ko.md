...
> ⚠️ **버전 안내 (먼저 읽어주세요):** 해피해키랩(HappyHackyLab)에서 만든 이 패키지는 **OpenWebUI 0.9.6 버전 기준으로 최신화**되어 있습니다. 다른(더 최신) 릴리스가 필요하면 [OpenWebUI 공식 사이트·GitHub](https://github.com/open-webui/open-webui)를 방문해 직접 업그레이드하거나 해당 버전의 호환성을 확인하세요. 상위 버전에서는 일부 UI·API가 달라 도구 등록이나 캘린더 동작이 바뀔 수 있습니다.
...
- [🧠🧠 VoiceMem 이중뇌 구조 (좌뇌·우뇌 기억)](#-voicemem-이중뇌-구조-좌뇌우뇌-기억)
- [📍 위치 확인 (NAVER + 카카오 + 구글)](#-위치-확인-naver--카카오--구글)
- [🛰️ GPS 위치 공유 (노인 보호)](#️-gps-위치-공유-노인-보호)
...
| 🧠🧠 VoiceMem 이중뇌 | 기억의 앵커를 떠받치는 기억 엔진. **좌뇌(사실·구조: 누가·언제·무엇)** 와 **우뇌(감정·맥락: 분위기·연상)** 로 통화 기억을 나눠 저장·회상. 켜고 끌 수 있는 선택 부품이며, 임베딩·분석에 **Ollama(로컬) 또는 OpenAI API** 중 하나를 골라 사용 |
| 📍 위치 확인 (NAVER + 카카오 + 구글) | 안부전화 중 상대방 **동의를 받은 뒤** 위치를 물어보고, 말로 알려준 위치를 지도 이미지와 함께 **텔레그램으로 보고**. NAVER 지오코딩(도로명·지번) + 카카오 로컬(역명·상호·건물명)으로 좌표를 찾고, 지도 이미지는 **구글 hybrid 위성사진**(구글 키 없으면 NAVER 일반지도). 휴대폰 GPS를 몰래 수집하지 않음 |
| 🛰️ GPS 위치 공유 (노인 보호) | 대상에게 **위치 공유 링크를 전화+문자로 발송**. 대상이 링크를 열고 **직접 [허용]을 눌러 동의하면** 실시간 GPS 위치가 관리자 텔레그램·문자로 전송. 전화만으로 GPS를 못 가져오는 한계를 링크+브라우저 위치 허용으로 해결. 보안 5겹(랜덤 토큰·일회성·시간제한·HTTPS·대상번호 연결) |
...
`start-openwebui-VoiceMem-integrated.sh`는 **설치를 시작하면 가장 먼저** 용도를 묻습니다.
...
- (선택) VoiceMem 이중뇌 기억용 — **Ollama(로컬, 무료)** 또는 **OpenAI API 키** 중 택1. OpenAI를 고르면 Ollama 설치 없이 임베딩·분석이 클라우드에서 처리됩니다
...
less start-openwebui-VoiceMem-integrated.sh    # 실행 전 내용 검토 권장
...
| NAVER 위치 확인 | `y` / `N` | `y`면 NAVER Maps Client ID·Secret 입력 → 안부전화 중 위치 확인·지도 보고 활성화. 이어서 (선택) 카카오 REST API 키(역명·상호 검색), (선택) 구글 Maps API 키(위성사진 지도)를 입력. `N`(기본)이면 위치 기능 미사용 |
...
> **📍 위치 확인도 선택입니다.** "NAVER 지도 위치 확인 기능을 사용하시겠습니까? (y/N)"에서 `y`를 누르면 NAVER Maps 키를 입력받아 위치 확인 기능이 켜집니다. 이어서 카카오 REST API 키를 넣으면 역명·상호까지 검색됩니다(선택). 위치 보고는 **텔레그램**으로 지도 이미지와 함께 오므로, 위치 기능을 쓰려면 텔레그램 보고를 함께 설정하는 것을 권합니다. 자세한 내용은 [위치 확인 (NAVER + 카카오 + 구글)](#-위치-확인-naver--카카오--구글) 참고.

설치가 끝나면 OpenWebUI, 전화 봇, RAG, 실시간 음성 서버 등 전체 스택이 자동으로 뜨고 **도구 14개(캘린더·실시간 귓속말·음성엔진 설정·기억의 앵커·NAVER 위치 확인·GPS 위치 공유 포함)가 자동 등록**됩니다.
...
**OPENWEBUI_API_KEY 값 찾는 법:**

3단계의 키는 웹에서 **설정 → 계정 → API 키**로 발급하는 게 기본이지만, 설치 시 자동 발급된 값이 `.env`에 있으면 서버에서 바로 확인할 수 있습니다.

```bash
# .env 에서 OpenWebUI API 키 확인 (이 값을 밸브에 붙여넣기)
grep OPENWEBUI_API_KEY ~/OpenWebUI/.env
```

출력 예시:
```
OPENWEBUI_API_KEY=eyJhbGciOi...   (JWT 토큰) 또는  sk-...
```

`=` 뒤의 값을 복사해 **캘린더 밸브의 `OPENWEBUI_API_KEY`** 칸에 붙여넣고 저장하면 됩니다. (같은 값을 브라우저 에이전트 도구 밸브에도 넣습니다.)

> `.env`에 이 값이 없거나 비어 있으면 아직 발급 전입니다. 그때는 위 3단계대로 웹에서 발급하세요. 발급된 키가 `eyJ...`(JWT)면 만료가 있으니 장기 사용에는 `sk-...` 형식을 권장합니다([API 키 형식](#api-키-형식) 참고).
...
### OpenWebUI API 키를 서버에서 확인 (`OPENWEBUI_API_KEY`)

캘린더·브라우저 도구 밸브의 `OPENWEBUI_API_KEY` 칸에 넣는 키입니다. 웹에서 **설정 → 계정 → API 키**로 발급하는 게 기본이지만, 설치 과정에서 자동 발급된 값이 `.env`에 들어 있는 경우 서버에서 바로 확인할 수 있습니다.

```bash
# .env 에서 OpenWebUI API 키 확인
grep OPENWEBUI_API_KEY ~/OpenWebUI/.env
```
...
`=` 뒤의 값을 복사해, **워크스페이스 → 도구 → 캘린더/브라우저 에이전트 → ⚙️ 밸브 → `OPENWEBUI_API_KEY`** 칸에 붙여넣고 저장하면 됩니다. 두 도구에 **같은 값**을 넣습니다.

> `.env`에 이 값이 없거나 비어 있으면 아직 발급 전입니다. 그때는 웹에서 **설정 → 계정 → API 키**로 새로 발급한 뒤 두 도구 밸브에 넣으세요. 발급된 키가 `eyJ...`(JWT)면 만료가 있으니, 장기 사용에는 `sk-...` 형식을 권장합니다([API 키 형식](#api-키-형식) 참고).
...
## 🧠🧠 VoiceMem 이중뇌 구조 (좌뇌·우뇌 기억)

**VoiceMem**은 위 [기억의 앵커](#-기억의-앵커-통화-회상--능동형-자동전화)를 떠받치는 기억 엔진입니다. 사람의 뇌가 좌·우로 역할을 나누듯, 하나의 통화 기억을 **두 갈래로 나눠 저장하고 회상**합니다. 켜고 끌 수 있는 **선택 부품**이라, 필요 없으면 꺼도 나머지 기능은 그대로 작동합니다.

### 왜 이중뇌인가

기억을 한 덩어리로만 저장하면 "정확한 사실"과 "그때의 분위기"가 뒤섞여, 회상할 때 둘 중 하나를 놓치기 쉽습니다. VoiceMem은 이 둘을 분리해 각각 가장 잘하는 방식으로 저장합니다.

| 구분 | 담당(뇌) | 저장하는 것 | 회상에 쓰는 방식 |
|------|---------|------------|-----------------|
| **좌뇌 (Left)** | 사실·구조 | **누가·언제·무엇** — 전화번호·이름·날짜·시간대·주제·약속·요청사항 등 구조적 사실 | 정확한 키(전화번호·날짜·주제)로 조회 |
| **우뇌 (Right)** | 감정·맥락 | **분위기·감정·연상** — 감정 상태, 대화 톤, 날씨, 시간대의 느낌 등 맥락 | 의미 임베딩으로 "지금과 비슷한 분위기"를 유사도 검색 |

회상할 때는 두 뇌의 결과를 **합쳐서** AI에게 전달합니다. 좌뇌가 "이 사람과 지난주 화요일에 이직 얘기를 했다"는 사실을, 우뇌가 "그때 비 오는 저녁이라 좀 가라앉은 분위기였다"는 맥락을 각각 떠올려, AI가 "지난번 이직 고민은 좀 정리되셨어요?"처럼 사실과 분위기를 모두 담아 자연스럽게 이어갑니다.
...
[통화 종료]
  ↓
VoiceMem 이 통화 내용을 분석
  ├─ 좌뇌 저장: 전화번호·이름·날짜·시간대·주제·약속 (구조적 사실)
  └─ 우뇌 저장: 감정·톤·날씨·맥락 → 의미 임베딩 (벡터)
        ↓
[다음 통화 시작]
  ↓
같은 전화번호로 두 뇌를 동시에 조회
  ├─ 좌뇌: 정확한 사실 목록
  └─ 우뇌: 지금 시간대·분위기와 비슷한 과거 기억 (유사도 상위)
        ↓
두 결과를 합쳐 AI에게 전달 → AI가 사실+분위기를 담아 대화
```

### 엔진 선택 — Ollama(로컬) 또는 OpenAI API

VoiceMem의 두 뇌는 **임베딩 생성**(우뇌의 유사도 검색용)과 **통화 분석**(감정·주제·요약 추출)에 AI를 사용합니다. 이 둘을 처리할 엔진을 **하나 골라** 설정합니다.

| 엔진 | 임베딩 | 분석 | 특징 |
|------|--------|------|------|
| **Ollama** (기본·로컬) | `nomic-embed-text` (768차원) | 로컬 LLM | 무료·오프라인·프라이버시. WSL2에서 별도 설정 필요 ([아래 참고](#ollama-임베딩-자동-설정-wsl2)) |
| **OpenAI API** (선택·클라우드) | `text-embedding-3-small` 등 | GPT 모델 | Ollama 설치 불필요, 품질 안정적. 사용량만큼 과금 |

> **둘 중 하나만 씁니다(택1).** 설정에서 엔진을 바꾸면 임베딩·분석이 통째로 그쪽으로 전환됩니다. Ollama로 저장한 기억과 OpenAI로 저장한 기억은 임베딩 차원·모델이 달라 섞이지 않으므로, **엔진은 처음에 정해 일관되게 쓰는 것**을 권장합니다. 중간에 바꾸면 이전 기억의 우뇌(유사도) 회상 품질이 떨어질 수 있습니다(좌뇌의 사실 조회는 영향 없음).

#### OpenAI API 키 방식으로 켜기

`twilio-bot/ai_config.py`에서 엔진을 OpenAI로 지정합니다.

```python
# twilio-bot/ai_config.py
VOICEMEM_ENABLED = True             # VoiceMem 이중뇌 사용 (끄려면 False)
VOICEMEM_ENGINE = "openai"          # "ollama"(기본) 또는 "openai"
VOICEMEM_OPENAI_EMBED_MODEL = "text-embedding-3-small"   # 우뇌 임베딩 모델
VOICEMEM_OPENAI_CHAT_MODEL  = "gpt-4o-mini"              # 통화 분석(감정·주제·요약)
```

키는 `.env` 또는 Docker Secrets에 넣습니다(키가 코드·이미지에 남지 않도록 Secrets 권장).

```bash
# 방법 A) .env 에 추가
echo "OPENAI_API_KEY=sk-..." >> ~/OpenWebUI/.env

# 방법 B) Docker Secrets (권장)
...
> OpenAI 엔진을 켜면 **Ollama 설치·WSL2 바인딩이 필요 없습니다.** 임베딩과 분석이 모두 OpenAI에서 처리되므로, Ollama 연결 문제(더미 벡터 경고 등)와 무관하게 회상 품질이 안정적입니다. 대신 통화 저장·회상마다 OpenAI API 호출이 발생해 사용량만큼 과금되니, 요금을 주기적으로 확인하세요.

> **VoiceMem을 아예 끄려면** `VOICEMEM_ENABLED = False`. 이 경우 통화 기억 저장·회상이 비활성화되고, 나머지 통화·캘린더·텔레그램 기능은 정상 작동합니다.
...
**위치 확인이 주 목적인 통화** ("○○ 위치 물어봐줘")

```
[전화 받음]
  ↓
"안녕하세요, ○○님. 관리자님 부탁으로 전화드렸어요.
 관리자가 현재 위치를 여쭤보라고 하셨습니다. 위치를 알려주실 수 있으실까요?
 동의하시면 예, 알려주기 어려우시면 아니요라고 답해주세요."   ← 인사 후 바로 위치 동의
  ↓
[상대가 "예"] → "지금 어디에 계신지 말씀해 주세요"
      ↓  (상대가 "역삼동", "강남역" 등으로 답)
   NAVER 지오코딩 시도 → 실패 시 카카오 로컬 검색으로 폴백
      ↓  좌표 확인 → 구글 hybrid 위성사진 생성 (구글 키 없으면 NAVER 일반지도)
   📨 관리자 텔레그램으로 지도 이미지 + 주소 전송
      ↓
   "확인했습니다. 담당자에게 전달하겠습니다. 편안한 하루 보내세요." → 통화 종료
[상대가 "아니요"] → 위치 확인 건너뜀
```

**안부전화 중 위치도 물어본 경우** — 위치를 받은 뒤에도 안부 대화를 계속 이어갑니다.

- 위치 확인 통화는 **인사부터** 시작합니다(관리자 명령문이 그대로 첫마디가 되지 않도록). 인사에 이어 바로 위치 동의를 물으며, "요즘 어떻게 지내세요?" 같은 안부 질문은 붙이지 않습니다.
- **동의 질문은 최대 2번까지만** 반복합니다(무응답 시 무한 반복 방지). 2번 안에 예/아니요가 확인되지 않으면 위치 확인을 건너뜁니다.
- **순수 안부전화**(위치 언급 없는 "○○한테 안부전화 해줘")는 이 흐름과 무관하게 예전처럼 자연스럽게 대화합니다. 위치 동의 질문은 나오지 않습니다.
...
- 지도 이미지는 구글 hybrid 위성사진을 사용합니다(구글 키가 없으면 NAVER Static Map 일반지도로 폴백).
...
좌표 변환은 NAVER/카카오가 하고, **최종 지도 이미지**는 구글 hybrid 위성사진(위성 + 지명 글자)으로 만들 수 있습니다. 구글 Maps 키를 넣으면 텔레그램에 오는 지도가 위성사진으로 바뀝니다.

- **구글 키 있음** → 구글 hybrid 위성사진 (건물 지붕·실제 지형 + 도로명·지명)
- **구글 키 없음/실패** → NAVER 일반지도로 자동 폴백 (안전장치)

> hybrid는 위성사진 위에 지명·도로명을 겹쳐 보여줘 "어디인지" 파악이 쉽습니다. 순수 위성사진(글자 없음)을 원하면 코드의 `"maptype": "hybrid"` 를 `"satellite"` 로 바꾸면 됩니다.

**구글 Maps API 키 발급:**
1. [Google Cloud Console](https://console.cloud.google.com) 로그인 → 프로젝트 선택/생성
2. **API 및 서비스 → 라이브러리** → **Maps Static API** 검색 → **사용 설정**
3. **결제 계정 연결** (필수) — 월 $200 무료 크레딧 제공, 위치 확인 용도로는 사실상 무료
4. **API 및 서비스 → 사용자 인증 정보 → API 키 만들기** → `AIza…` 키 복사
5. (권장) 그 키에 **API 제한 → Maps Static API만 허용** 설정으로 오남용 방지

> ⚠️ 구글 키는 결제와 연결되므로, 유출 시 요금이 발생할 수 있습니다. 키에 API 제한과 사용량 알림(예산 알림)을 걸어두는 것을 권합니다.

### 준비물 — API 키 발급

**1) NAVER Maps (필수)** — 좌표 변환 + 지도 폴백
...
**3) 구글 Maps (선택 · 위성사진 지도용)**
1. [Google Cloud Console](https://console.cloud.google.com) 로그인 → 프로젝트 선택/생성
2. **Maps Static API** 사용 설정 + **결제 계정 연결**(월 $200 무료 크레딧)
3. **API 키** 발급(`AIza…`) → (권장) Maps Static API로 제한
   - 넣으면 지도가 구글 hybrid 위성사진으로, 없으면 NAVER 일반지도로 옵니다. (자세히는 위 [지도 이미지 — 구글 위성사진](#지도-이미지--구글-위성사진-선택))

### 키 입력 — 설치 중 또는 나중에

설치 중 "NAVER 지도 위치 확인 기능을 사용하시겠습니까?"에서 `y`를 누르고 NAVER Client ID·Secret과 (선택) 카카오 REST API 키, (선택) 구글 Maps API 키를 순서대로 입력하면 됩니다. 키는 **Docker Secrets 파일**로 안전하게 저장됩니다.
...
# 구글 위성지도(선택)
echo -n "발급받은_구글_Maps_API_키"     >  secrets/google_maps_api_key
...
> `NAVER_MAPS_CLIENT_ID`는 `.env`(환경변수), `Secret`·카카오 키·구글 키는 `secrets/` 파일로 관리됩니다. 코드가 환경변수 → secrets 파일 순으로 읽으므로, gunicorn 워커가 환경을 상속하지 못하는 경우에도 파일에서 안전하게 불러옵니다.
...
위치 확인(NAVER/카카오/구글)이 **통화 중 말로 알려준 위치**를 쓰는 것과 달리, 이 14번째 도구는 대상이 **직접 [허용]을 눌러 동의하면 실시간 GPS 좌표**를 관리자에게 전송합니다. 홀로 계신 부모님·어르신의 안전 확인처럼, 정확한 실시간 위치가 필요한 노인 보호 목적에 적합합니다.

> ⚠️ **왜 링크 방식인가:** 일반 음성통화만으로는 상대 휴대폰의 GPS 센서에 접근할 수 없습니다(모든 전화의 물리적 한계). 그래서 이 기능은 **문자 링크 + 브라우저 위치 허용** 방식을 씁니다. 대상이 링크를 열고 위치 권한을 허용해야만 좌표가 전송되므로, **본인 동의가 항상 전제**됩니다.

### 동작 흐름

```
① 관리자: "박익제 GPS 위치 확인해줘"  (또는 번호로 지정)
   ↓
② 시스템이 대상에게 전화 + 문자를 함께 발송
   전화: "관리자님이 위치를 확인하려 하세요. 문자 링크를 눌러주세요."
   문자: "위치 공유: https://<도메인>/gps-share/<토큰>"
   ↓
③ 대상이 링크를 열면 안내 페이지 →
   "이 위치가 관리자님께 한 번 전송됩니다" 고지 + [📍 내 위치 보내기] 버튼
   ↓
④ 버튼 탭 → 브라우저가 "위치 허용?" → [허용]
   ↓
⑤ 실시간 GPS 좌표 → 관리자 텔레그램(지도 이미지) + 관리자 문자(좌표·지도 링크) 전송 ✅
```

대상 입장에서는 **전화 받기 → 문자 링크 탭 → "위치 보내기" 탭 → "허용" 탭**이면 끝입니다.

### 사용법

OpenWebUI 채팅이나 관리자 통화에서 자연어로 요청합니다.

- `"박익제 GPS 위치 확인해줘"` → 연락처에서 번호를 찾아 링크 발송
- `"010-1234-5678로 위치 공유 링크 보내줘"` → 번호로 직접 발송

발송되면 "○○님에게 위치 공유 링크를 보냈습니다(전화+문자). 30분 내 동의 시 위치가 전송됩니다"라고 안내합니다.

### 보안 (5겹)

| 겹 | 내용 |
|----|------|
| 랜덤 토큰 | 링크 주소를 추측 불가능한 긴 무작위 문자열로 생성 |
| 일회성 | 위치를 한 번 전송하면 그 링크는 즉시 만료(재사용 불가) |
| 시간제한 | 기본 30분 후 자동 만료 (미사용 시) |
| HTTPS | 전송 구간 암호화 (도메인이 https일 때) |
| 대상번호 연결 | 각 링크를 특정 대상 번호에 묶어, 위치를 받을 때 **누구 것인지** 명확히 하고 다른 사람과 섞이지 않게 함 |

> 유효기간(기본 30분)은 코드의 `GPS_SHARE_TTL_SEC` 값으로 조정할 수 있습니다.

### 법적·윤리적 유의

이 기능은 **본인이 링크에서 직접 동의(허용)**해야만 위치가 전송되도록 설계되어, 동의 기반 위치 이용이라는 원칙에 부합합니다. 다만 다음을 지켜 주세요. (아래는 일반 안내이며 법률 자문이 아닙니다. 실제 적용 전 전문가 확인을 권장합니다.)

- **명확한 고지·동의**: 대상이 "무엇을, 누구에게 보내는지" 알고 눌러야 합니다. 공유 페이지에 이를 안내합니다.
- **판단능력**: 대상이 동의 능력이 약한 경우(중증 치매 등)에는 본인 동의만으로 부족할 수 있으며, 보호자·법정대리인 관련 사항을 확인해야 합니다.
- **몰래 추적 금지**: 대상 모르게 또는 속여서 위치를 받는 것은 위법입니다. 반드시 알리고 동의를 받으세요.
- **목적 제한**: 노인 보호·안전 확인 목적으로만 사용하고, 제3자 제공이나 다른 용도로 쓰지 마세요.

### 서버에 적용 (기존 설치에 추가)

전체 재설치 없이 GPS 기능만 추가하려면, 함께 제공된 `patch-gps-14th-tool.sh`를 설치 폴더에서 실행합니다. (백업 → 코드 삽입 → 재빌드까지 자동)
...
> 백엔드(전화·문자·GPS·텔레그램)는 이 패치로 반영됩니다. OpenWebUI의 도구 등록은 전체 재설치 시 자동으로 되며, 패치만 한 경우 관리자 화면에서 `gps_location_share` 도구를 수동 추가할 수 있습니다.
...
대상에게 전화+문자가 가고, 대상이 링크에서 [허용]을 누르면 관리자 텔레그램·문자로 위치가 도착합니다.

---

## Ollama 임베딩 자동 설정 (WSL2)

기억의 앵커·연락처 의미검색은 기본적으로 **Ollama 임베딩**(`nomic-embed-text`, 768차원)을 사용합니다. 설치 스크립트가 다음을 **자동 처리**합니다:

> **로컬 Ollama 대신 OpenAI를 쓸 수도 있습니다.** VoiceMem 이중뇌 엔진을 OpenAI API로 전환하면 아래 Ollama 설정은 건너뛰어도 됩니다(임베딩·분석이 클라우드에서 처리). 자세히는 [VoiceMem 이중뇌 구조 — 엔진 선택](#-voicemem-이중뇌-구조-좌뇌우뇌-기억) 참고.
...
- 디렉토리 구조 / 필수 파일 / 권한
- Docker 컨테이너 상태 / 네트워크
- 도구 14개 등록 (캘린더·실시간 귓속말·음성엔진 설정·기억의 앵커·NAVER 위치 확인·GPS 위치 공유 포함)
...
├── start-openwebui-VoiceMem-integrated.sh                  # 메인 설치 (고객/개인 선택 · SERVICE_MODE 전환)
├── setup-browser-agent-calendar-naver-search-api-only.sh   # 브라우저 에이전트 + 캘린더
...
A. 아니요. 일반 음성통화는 상대방 단말의 GPS 센서에 접근할 수 없습니다(모든 전화 시스템의 물리적 한계). 대신 **GPS 위치 공유** 도구는 문자 링크를 보내 대상이 브라우저에서 [허용]을 누르면 실시간 GPS를 전송합니다(본인 동의 필수). → [GPS 위치 공유](#️-gps-위치-공유-노인-보호)

**Q. GPS 위치 공유는 상대방 동의 없이도 되나요?**
A. 안 됩니다. 그리고 그렇게 설계하지도 않았습니다. 대상이 링크를 열고 직접 [허용]을 눌러야만 위치가 전송됩니다. 몰래·강제 추적은 위법이며, 이 기능은 노인 보호처럼 **동의 기반**으로만 사용해야 합니다.

**Q. GPS 위치 링크의 유효기간을 바꾸고 싶어요.**
A. 기본 30분입니다. `twilio-bot/twilio_bot.py`의 `GPS_SHARE_TTL_SEC`(초 단위)를 바꾸고 봇을 재빌드하세요. 보안을 위해 너무 길게 두지 않는 것을 권합니다.

**Q. 위치 보고에 좌표만 오고 지도 이미지가 안 와요.**
A. 지도 이미지 저장 폴더(`twilio-bot/data/reports`) 권한 문제이거나 지도 API 호출 실패일 수 있습니다. 최신 스크립트는 설치 시 폴더 소유권을 봇 사용자로 맞춰 해결합니다. 재설치하거나 `docker compose up -d --build twilio-bot`로 재빌드하세요.

**Q. 지도를 위성사진으로 받고 싶어요.**
A. 구글 Maps API 키를 넣으면 지도가 구글 hybrid 위성사진(위성 + 지명)으로 옵니다. 설치 중 NAVER 위치 기능을 켠 뒤 구글 키를 입력하거나, `secrets/google_maps_api_key` 파일에 키를 넣고 재빌드하세요. 구글 키가 없으면 NAVER 일반지도로 옵니다. → [지도 이미지 — 구글 위성사진](#지도-이미지--구글-위성사진-선택)

**Q. 위성사진인데 일반지도처럼 보여요.**
A. 기본값 hybrid는 위성사진 위에 도로명·지명 글자를 겹쳐 보여줍니다. 도심은 글자·도로가 빽빽해 언뜻 일반지도처럼 보일 수 있지만, 실제로는 건물 지붕·지형이 사진으로 찍혀 있습니다. 글자 없는 순수 위성사진을 원하면 코드의 `"maptype": "hybrid"` 를 `"satellite"` 로 바꾸세요.

**Q. 위치 통화인데 인사도 없이 "위치가 어디에요"로 시작해요.**
A. 예전에는 관리자 명령문이 그대로 첫마디가 됐습니다. 최신 버전은 위치 통화가 항상 "안녕하세요, ○○님. 관리자님 부탁으로 전화드렸어요…"라는 인사로 시작한 뒤 위치 동의를 묻습니다. 재설치하거나 봇을 재빌드하세요.

**Q. 위치를 물어야 하는데 AI가 "잘 지내시죠?" 같은 안부만 물어요.**
A. 최신 버전은 위치 확인 통화에서 안부 질문을 붙이지 않고, 인사 직후 바로 위치 동의를 묻도록 정리했습니다. (순수 안부전화는 반대로 안부 대화를 그대로 유지합니다.) 재빌드하면 반영됩니다.

**Q. "강남역" 같은 역명을 못 찾아요.**
A. NAVER 지오코딩은 도로명·지번 주소만 찾고 역명·상호는 못 찾습니다(NAVER 지역검색 API 종료). 카카오 REST API 키를 넣으면 역명·상호까지 검색됩니다. → [위치 확인 (NAVER + 카카오 + 구글)](#-위치-확인-naver--카카오--구글)
