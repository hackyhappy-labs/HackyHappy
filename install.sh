...
def ask_openwebui(user_input, system_prompt=None):
    try:
        if not system_prompt:
            system_prompt = "당신은 친절한 AI 전화 어시스턴트입니다. 짧고 명확하게 한국어로 답변하세요. 2~3문장 이내로 답하세요."
        res = requests.post(
...
def validate_twilio_request(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not validator.validate(url, post_vars, signature):
            print(f"🚨 Twilio 서명 검증 실패! 위조 요청 차단: {request.remote_addr}")
...
def make_call(to_number, custom_message=None, contact_name=None, mission=None, report_to=None):
    ...
    try:
        ...
        url = f"{SERVER_DOMAIN}/voice-out"
        params = {}
        if custom_message:
            params["msg"] = custom_message
        if contact_name:
        ...
        call = client.calls.create(to=to_number, from_=TWILIO_PHONE, url=url)
        ...
        outbound_sessions[call.sid] = {
            ...
            "history"     : [],
            "summary_sent": False,
            "report_to"   : report_to or MY_PHONE,   # 보고 받을 번호
        }
        print(f"📞 아웃바운드 세션 생성: {call.sid} → {contact_name}({to_number}) | 보고→{report_to or MY_PHONE}")
        return call.sid
    except Exception as e:
        print(f"전화 걸기 오류: {e}")
        return None
...
@validate_twilio_request
def voice_out():
    """아웃바운드 전화 - 안부전화 전용 대화 흐름"""
    msg          = request.args.get("msg", "안녕하세요. AI 어시스턴트입니다.")
    ...
    if contact_name and mission:
        # 안부전화 모드: 임무 기반 대화
        action_url = f"{SERVER_DOMAIN}/respond-out?name={requests.utils.quote(contact_name)}&mission={requests.utils.quote(mission)}&call_sid={call_sid}"
        gather = Gather(input="speech", action=action_url,
                        language="ko-KR", timeout=6, speechTimeout="auto")
        gather.say(msg, language="ko-KR")
        response.append(gather)
        ...
        gather = Gather(input="speech", action=f"{SERVER_DOMAIN}/respond",
                        language="ko-KR", timeout=5, speechTimeout="auto")
        gather.say(msg, language="ko-KR")
        gather.say("용건이 있으시면 말씀해 주세요.", language="ko-KR")
        response.append(gather)
...
def respond_out():
    ...
    contact_name = request.args.get("name", "상대방")
    mission      = request.args.get("mission", "안부 확인")
    response     = VoiceResponse()
    ...
    session = outbound_sessions.get(call_sid, {
        ...
        "history": [], "summary_sent": False
    })
    outbound_sessions[call_sid] = session

    if not user_speech:
        gather = Gather(input="speech",
                        action=f"{SERVER_DOMAIN}/respond-out?name={requests.utils.quote(contact_name)}&mission={requests.utils.quote(mission)}&call_sid={call_sid}",
                        language="ko-KR", timeout=6, speechTimeout="auto")
        gather.say("잘 듣지 못했습니다. 다시 말씀해 주세요.", language="ko-KR")
        response.append(gather)
        return Response(str(response), mimetype="text/xml")
...
# 기존 설정에 /voice /respond /call-me 블록 추가 (중복 방지)
