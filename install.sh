...
@app.post("/tools/contacts/delete")
...
echo ""
echo "┌────────────────────────────────────────────┐"
echo "🔐 OpenWebUI 시작 대기 중..."
echo "└────────────────────────────────────────────┘"

# OpenWebUI 완전히 뜰 때까지 최대 120초 대기
OW_READY=false
for i in $(seq 1 24); do
  STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/health 2>/dev/null)
  if [ "$STATUS" = "200" ]; then
    OW_READY=true
    echo "   ✅ OpenWebUI 준비 완료! (${i}회 시도)"
    break
  fi
...
if [ "$OW_READY" = false ]; then
  echo "   ⚠️  OpenWebUI 시작 실패 → 로그 확인: docker compose logs open-webui"
fi

echo ""
echo "┌────────────────────────────────────────────┐"
echo "🔐 OpenWebUI 계정 생성 + API 키 발급 중..."
echo "└────────────────────────────────────────────┘"
...
if [ -n "$OW_JWT" ]; then
  ...
  # JWT 토큰을 API 키로 사용 (OpenWebUI 0.8+ 에서 api_key 엔드포인트 변경됨)
  OW_API_KEY="$OW_JWT"
  echo "   ✅ JWT 토큰을 API 키로 사용"
...
PYEOF
    ...
    # 이미 등록된 경우 구버전(phone_assistant) 정리 후 재등록
    if echo "$TOOL_RESULT" | grep -q "already registered"; then
      echo "   ⚙️  기존 Tool 정리 중..."
      python3 << PYEOF3
...
jwt = "$OW_JWT"
# 구버전 ID 목록 삭제 시도
for old_id in ["phone_assistant", "phone_assistant_v2"]:
    try:
        req = urllib.request.Request(
            f"http://localhost:3000/api/v1/tools/{old_id}/delete",
...
try:
    with urllib.request.urlopen(req) as resp:
        ...
        tool_id = None
        for t in tools:
            if t.get("id") == "phone_assistant_v2":
                tool_id = t.get("id")
                break
    ...
    if tool_id:
        ...
        req2 = urllib.request.Request(
            f"http://localhost:3000/api/v1/tools/{tool_id}",
...
PYEOF2
      ...
      echo "   ⚠️  Tool 자동 등록 실패 → 수동 등록 명령어:"
      echo "      OW_JWT=\$(curl -s -X POST http://localhost:3000/api/v1/auths/signin \\"
      echo "        -H 'Content-Type: application/json' \\"
      echo "        -d '{\"email\":\"${OW_EMAIL}\",\"password\":\"${OW_PASSWORD}\"}' \\"
      echo "        | grep -o '\"token\":\"[^\"]*\"' | cut -d'\"' -f4)"
      echo "      python3 /tmp/register_tool.py \$OW_JWT"
    fi
  else
    echo "   ⚠️  API 키 적용 실패 → 수동 설정 필요"
  fi
else
  echo "   ⚠️  로그인 실패 → 수동 등록 명령어:"
  echo "      OW_JWT=\$(curl -s -X POST http://localhost:3000/api/v1/auths/signin \\"
  echo "        -H 'Content-Type: application/json' \\"
  echo "        -d '{\"email\":\"${OW_EMAIL}\",\"password\":\"${OW_PASSWORD}\"}' \\"
  echo "        | grep -o '\"token\":\"[^\"]*\"' | cut -d'\"' -f4)"
