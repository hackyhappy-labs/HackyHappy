...
class Tools:
    ...
    TOOL_RESP=$(curl -s -X POST http://localhost:3000/api/v1/tools/create \
      -H "Authorization: Bearer ${OW_JWT}" \
      -H "Content-Type: application/json" \
      -d "{
        \"name\": \"전화 어시스턴트\",
        \"description\": \"전화 걸기, 안부전화, 연락처 조회\",
        \"content\": $(echo "$TOOL_CODE" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))')
      }")

    if echo "$TOOL_RESP" | grep -q '"id"'; then
