...
def _estimate_speech_seconds(text):
    ...
    if not text:
        return 0.0
    n = len(text)
    # 분당 300자 ≈ 초당 5자. 최소 1초 보장.
    return max(1.0, n / 5.0)
...
def detect_barge_in(call_sid):
    ...
    True면 끼어들기. 동시에 추적 상태는 소비(삭제)한다.
    ...
    # 예상 낭독 시간의 (BARGEIN_THRESHOLD) 이전에 사용자가 말했고,
    # 답변이 충분히 길었을(BARGEIN_MIN_SECONDS 이상) 때만 끼어들기로 본다.
...
def voice():
    ...
    if is_admin(caller):
        ...
        gather = Gather(input="speech", barge_in=True, speechModel="phone_call", enhanced="true", action=f"{SERVER_DOMAIN}/respond",
...
def voice_out_simple():
    ...
    tts_lang = get_twilio_lang(lang)
    tts_voice = get_tts_voice(lang)
    gather = Gather(input="speech", barge_in=True, speechModel="phone_call", enhanced="true", action=f"{SERVER_DOMAIN}/respond-admin",
                    language=tts_lang, timeout=TIMEOUT_INBOUND, speechTimeout="auto")
    gather.say(get_msg("GREETING_ADMIN", lang), voice=tts_voice, language=tts_lang)
    response.append(gather)
...
질문:"""
    ...
    try:
        ...
        speech_to = "auto"
    gather = Gather(input="speech", barge_in=True, speechModel="phone_call", enhanced="true", action=action_url,
...
def respond_out():
    ...
    if not user_speech:
        ...
        if patience < max_patience:
            ...
            gather = Gather(input="speech", barge_in=True,
