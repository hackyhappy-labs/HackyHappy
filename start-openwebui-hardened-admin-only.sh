...
# BARGEIN_CPS: 언어별 TTS 실측 낭독 속도 (초당 글자수)
...
SPEECH_TIMEOUT_OUTBOUND = "auto"   # 🔧 2초→auto: 상대방이 말을 멈추면 즉시 응답 (빠른 대화)
                                   # (auto가 말 끝남을 자동 감지 — 더듬는 경우 망설임 감지 로직이 별도 보호)
...
description: OpenWebUI 내장 캘린더에서 오늘 일정을 조회합니다. 밸브에 API 키 입력.
version: 1.0.0
"""
import datetime
import requests
...
class Tools:
    ...
    def get_today_schedule(self, __user__: dict = {}) -> str:
        ...
        # 전화 봇과 키 공유: 밸브 키를 공유 폴더에 동기화 (전화 봇이 이 파일을 읽음)
        try:
            import os as _os
            ...
            if _prev != key:
                with open(share_path, "w") as _f:
                    _f.write(key)
                try:
                    # 전화 봇(다른 UID)도 읽을 수 있도록 644 + 폴더 진입 가능하게 755
                    # (이 폴더는 컨테이너 내부 볼륨이라 외부 노출 없음)
...
print("9️⃣  캘린더 (오늘 일정) Tool 등록 중...")
result9 = register_tool("calendar_today", "캘린더 (오늘 일정)", "OpenWebUI 내장 캘린더에서 오늘 일정을 조회. 밸브에 API 키 입력.", calendar_tool_code)
print(result9)
PYEOF
