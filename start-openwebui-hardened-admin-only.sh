...
#
# ✅ 자연스러운 끼어들기 (barge-in)
#    - AI가 말하는 도중 상대방이 말을 시작하면 AI가 즉시 멈추고 다시 듣고 답함
#    - 사람과 통화하듯 자연스러운 대화 흐름 (모든 음성 응답에 적용)
#    - ※ 주변 소음이 큰 환경에서는 잡음에 반응할 수 있음 (조용한 환경 권장)
#
# ✅ RAG 자동 등록
#    - 문서 검색 Tool 자동 등록
#
...
# 통화 PIN 인증은 제거되었습니다 (등록된 관리자 번호만 허용).
# ADMIN_PIN 변수는 호환성 유지를 위해 자동 생성만 하고, 통화에는 사용되지 않습니다.
ADMIN_PIN=$(od -An -N3 -tu4 /dev/urandom 2>/dev/null | tr -d ' \n')
ADMIN_PIN=$(printf '%06d' $(( ${ADMIN_PIN:-0} % 1000000 )) 2>/dev/null || echo "519740")
echo "   ✅ 통화 인증: 등록된 관리자 번호만 허용 (PIN 입력 불필요)"
...
# AI가 답변을 시작한 시각과 그 답변 내용을 기록해두고,
# 다음 사용자 발화가 "예상 낭독 시간"보다 빨리 들어오면 끼어들기로 판단한다.
...
# Twilio 는 Gather 반환 시 Confidence(0~1)를 함께 POST 한다.
...
def _estimate_speech_seconds(text, lang="ko"):
    ...
    언어별 실측 속도(BARGEIN_CPS)를 쓰고, 문장부호(쉼표/마침표/물음표)마다
    짧은 정지 시간을 더해 실제 낭독 길이에 가깝게 추정한다."""
    if not text:
        return 0.0
    cps = BARGEIN_CPS.get(lang, BARGEIN_CPS.get("ko", 4.5))
    base = len(text) / cps
    # 문장부호에서 생기는 자연스러운 쉼(각 0.25초) 반영
...
def detect_barge_in(call_sid, confidence=None):
    ...
    판단 근거(정확도 개선):
      1) 실측 경과시간 vs 실측 기반 예상 낭독시간 → 답변 도중에 끊고 들어왔는지
      2) Twilio Confidence(음성인식 신뢰도) → 낮으면 말이 겹친 정황(보조 가점)
    임계값은 ai_config.py 의 BARGEIN_THRESHOLD / BARGEIN_MIN_SECONDS 로 조절.
    끼어들기 횟수 제한은 없다 (통화 내내 몇 번이든 인정)."""
    import time as _t
    ...
    # 너무 짧은 답변("네", "알겠습니다")은 끼어들 일이 거의 없으므로 제외
    if spoken_est < BARGEIN_MIN_SECONDS:
        return False

    # ── 1) 시간 기반 판단: 낭독 예상시간의 THRESHOLD 비율 이전에 말이 돌아왔는가 ──
    time_bargein = elapsed < (spoken_est * BARGEIN_THRESHOLD)

    # ── 2) 신뢰도 보조 판단: 겹쳐 말하면 STT 신뢰도가 낮게 나오는 경향 ──
    #     시간상 '경계선'(THRESHOLD~0.85 구간)이라도 신뢰도가 낮으면 끼어들기로 인정
    conf_bargein = False
    ...
    if is_bargein:
        ...
        print(f"✋ 끼어들기 인정 (근거:{_reason}, "
              f"경과 {elapsed:.1f}s / 예상 {spoken_est:.1f}s): {call_sid}")
    return is_bargein
...
def mark_history_interrupted(call_sid):
    ...
    이렇게 하면 AI가 '내 답변이 끝까지 전달되지 않았다'는 맥락을 이해한다."""
...
def _calendar_event_exists(title, start_epoch, tolerance_sec=120):
    ...
    - title: 등록 당시 제목
    - start_epoch: 일정 시작 시각(초)
    - tolerance_sec: 시작시각 허용 오차(초). 기본 2분.
    반환: True(존재)/False(없음/삭제됨). 조회 실패 시엔 안전하게 True(발송 유지)."""
    key = _read_calendar_key()
    if not key:
        return True  # 키 문제로 확인 불가 → 알림은 유지(오작동 방지)
    try:
        ...
        for e in events:
            ...
            try:
                ...
                e_start = 0
            # 제목 일치 + 시작시각이 허용 오차 이내이면 "아직 존재"로 판단
            if e_title == _title and abs(e_start - target_ns) <= tol_ns:
                return True
        return False  # 일치하는 일정 없음 → 삭제된 것으로 판단
    except Exception:
        return True  # 예외 시 안전하게 알림 유지
...
def process_admin_command(speech, caller, bargein_note=""):
    import datetime as _dt2  # 캘린더 등록용
...
def voice():
    ...
    # 등록된 관리자 번호만 허용 — 그 외 모든 번호는 즉시 차단 (PIN 없음)
    print(f"🚫 미등록 번호 차단 (관리자 번호만 허용): {caller}")
...
@validate_twilio_request
def verify_pin():
    # PIN 인증은 제거되었습니다. 등록된 관리자 번호만 인정합니다.
    # (이 엔드포인트로 직접 들어와도 권한을 얻을 수 없도록 즉시 종료)
    caller   = request.form.get("From", "")
    response = VoiceResponse()
    lang     = get_lang_for_call(caller=caller)
    tts_lang = get_twilio_lang(lang)
    tts_voice = get_tts_voice(lang)
    print(f"🚫 verify-pin 비활성화 — 미등록 번호 차단: {caller}")
    response.say(get_msg("NOT_ADMIN", lang), voice=tts_voice, language=tts_lang)
    response.hangup()
    return Response(str(response), mimetype="text/xml")
...
def respond():
    ...
    # ── 끼어들기(barge-in) 감지 ──
    # 직전 AI 답변이 낭독을 마치기 전에 사용자가 말했다면 끼어든 것으로 본다.
    # Twilio 가 함께 보낸 음성인식 신뢰도(Confidence)를 보조 근거로 전달
    _confidence = request.form.get("Confidence", "")
    _interrupted = detect_barge_in(call_sid, confidence=_confidence)
    if _interrupted:
        # 직전 AI 답변을 '중간에 끊김'으로 표시 → AI가 맥락을 정확히 이해
...
try:
    ...
    def execute_schedule(s):
        ...
        # 📅 캘린더 일정 알림(셀프 리마인더): 관리자 본인에게 전화/SMS.
        if contact_name == "__self_reminder__":
            if not MY_PHONE:
                print("❌ 일정 알림 실패: 관리자 전화번호(MY_PHONE) 미설정")
                return
            # 🗑️ 발송 직전 확인: 캘린더에서 그 일정이 삭제됐으면 알림 안 보냄
            _ct = s.get("cal_title")
            _cs = s.get("cal_start")
    ...
    def calendar_reminder():
        ...
        try:
            ...
            return jsonify({"status": "error", "reason": "형식 오류"}), 400
        if reminder_min <= 0 or start_epoch <= 0:
            return jsonify({"status": "skipped", "reason": "알림 없음"})
        ...
        return jsonify({"status": "skipped", "reason": "알림 시각 지남 또는 번호 미설정"})
...
#   - 예: 0.8 로 올리면 더 민감 / 0.4 로 내리면 더 둔감
BARGEIN_THRESHOLD = 0.6

# BARGEIN_MIN_SECONDS: 이 시간(초)보다 짧은 답변은 끼어들기 판단에서 제외
#   - 짧은 답변("네", "알겠습니다")은 끼어들 일이 거의 없으므로 무시
#   - 3.0 = 3초 미만 답변은 끼어들기로 보지 않음 (권장 기본값)
BARGEIN_MIN_SECONDS = 3.0

# BARGEIN_ENABLED: 끼어들기 맥락 처리 자체를 켜고 끔
#   - True: 끼어들면 AI가 맞장구치고 새 질문에 집중 (권장)
#   - False: 끼어들기 맥락 처리 안 함 (단순 응답)
BARGEIN_ENABLED = True

# ─────────────────────────────────────────────────────
# 🎤 음성 인식 대기시간 (말이 끝났다고 판단하기까지 기다리는 시간)
# ─────────────────────────────────────────────────────
# 값이 클수록: 말 중간에 멈추거나 생각해도 봇이 성급하게 안 끊음 (여유롭게 말 가능)
# 숫자는 "초" 단위 문자열. (예: "5" = 5초 기다림)
SPEECH_TIMEOUT_ADMIN    = "5"   # 관리자 통화(일정 등록 등 긴 명령). 넉넉하게 권장.
SPEECH_TIMEOUT_INBOUND  = "3"   # 걸려온 전화 응대.
SPEECH_TIMEOUT_OUTBOUND = "3"   # 봇이 거는 안부전화.
#   - 예상 낭독 시간을 글자수÷속도 로 계산하는 데 사용합니다.
#   - Twilio Neural TTS 실측 기준 근사치. 목소리가 빠르면 값을 올리고, 느리면 내립니다.
#   - 이 값이 정확할수록 "답변 도중 끼어들었는지" 판단이 정확해집니다.
BARGEIN_CPS = {"ko": 4.5, "en": 14.0, "ja": 5.5, "zh": 4.0}

# BARGEIN_CONF_HINT: 음성인식 신뢰도 보조 판단 임계값 (0.0 ~ 1.0)
#   - 상대가 AI 말에 겹쳐 말하면 음성인식 신뢰도(Confidence)가 낮게 나오는 경향이 있습니다.
#   - 시간상 애매한 구간이라도 신뢰도가 이 값 이하이면 끼어들기로 인정(보조 근거).
#   - 오탐이 많으면 값을 낮추세요(예: 0.4), 끼어들기를 더 잘 잡고 싶으면 올리세요(예: 0.65).
BARGEIN_CONF_HINT = 0.55

# BARGEIN_NOTE: 끼어들기 감지 시 AI에게 주는 지시문 (말투/행동 조절)
#   - 이 문구를 바꾸면 끼어들었을 때 AI의 반응 스타일이 바뀝니다.
BARGEIN_NOTE = (
    "[상황: 사용자가 당신의 이전 답변 도중에 끼어들어 새로 말했습니다. "
    "이전 답변을 계속 이어가지 말고, 먼저 '네', '아 그러셨군요' 같은 짧은 맞장구로 "
    "받아준 뒤, 아래 사용자의 새 말에 정확히 집중해서 답하세요.]"
...
# ─── 7-1. 음성 인식 설정 ─────────────────────────────
# speechTimeout: 말을 멈춘 뒤 "끝났다"고 판단하기까지 기다리는 시간.
#   "auto"=즉시 감지(성급할 수 있음), 숫자(초)=그만큼 더 기다림(여유롭게 말 가능)
# ai_config.py 에서 조절 가능.
SPEECH_TIMEOUT_INBOUND  = globals().get("SPEECH_TIMEOUT_INBOUND", "3")   # 수신전화: 말 멈춰도 3초 대기
SPEECH_TIMEOUT_OUTBOUND = globals().get("SPEECH_TIMEOUT_OUTBOUND", "3")  # 발신전화: 3초 대기
SPEECH_TIMEOUT_ADMIN    = globals().get("SPEECH_TIMEOUT_ADMIN", "5")     # 관리자(긴 명령): 5초 대기
...
class Scheduler:
    ...
    def add(self, name, contact_name, action, schedule_time, repeat=None, mission="안부 확인", message="", enabled=True, report_to="", cal_title=None, cal_start=None):
        ...
        cal_title / cal_start: 캘린더 일정 알림인 경우, 발송 직전 존재 확인용
          (제목 + 시작시각 epoch초). 일정이 삭제됐으면 알림을 건너뛰기 위함.
        """
    ...
    def _should_run(self, s):
        ...
        if not s["repeat"]:
            ...
            try:
                ...
                # 📅 캘린더 알림은 예약 시각을 지나면 실행 안 함 (지난 알림 전화 방지)
                if s.get("contact_name") == "__self_reminder__":
                    return -45 < delta <= 5
                return abs(delta) < 45
            except Exception:
...
result9 = register_tool("calendar_today", "캘린더 (조회·등록)", "OpenWebUI 캘린더에서 오늘/특정 날짜 일정 조회 및 새 일정 등록(알림·설명 포함). 밸브에 API 키 입력.", calendar_tool_code)
print(result9)
PYEOF
