...
############################################
# 5-4. openwebui_tool.py — OpenWebUI 채팅 맥락 → Browser Agent 전달
# - __messages__ 에서 최근 대화 추출
# - browse / browse 계열 task 앞에 [이전 대화 맥락] 주입
# - Valves: CONTEXT_TURNS, MAX_CONTEXT_CHARS
############################################
step "5-4/9  OpenWebUI 채팅 맥락 전달 패치"

python3 - "${AGENT_DIR}" << 'CTX_PATCH'
import sys
...
# ── 1) Valves에 맥락 설정 추가 ──
VALVE_ANCHOR = '        MONITOR_ADMIN_ONLY: bool = Field(default=True, description="가격 모니터링 등록을 관리자만 허용 (SMS 요금 보호, 권장)")'
VALVE_NEW = VALVE_ANCHOR + '''
        CONTEXT_TURNS: int = Field(default=4, description="Browser Agent에 전달할 최근 대화 턴 수 (user+assistant 기준)")
        MAX_CONTEXT_CHARS: int = Field(default=1500, description="전달 맥락 최대 문자 수")
        ENABLE_CHAT_CONTEXT: bool = Field(default=True, description="OpenWebUI 채팅 맥락을 browse task에 포함")'''

if "CONTEXT_TURNS" in code:
    ok_list.append("맥락 Valves 이미 존재")
elif VALVE_ANCHOR in code:
    code = code.replace(VALVE_ANCHOR, VALVE_NEW, 1)
    ok_list.append("Valves: CONTEXT_TURNS / MAX_CONTEXT_CHARS / ENABLE_CHAT_CONTEXT")
else:
    print("  ⚠️  Valves 앵커 불일치 — 수동으로 CONTEXT_* 필드 추가 필요")

# ── 2) _build_chat_context 헬퍼 추가 (_set_user 앞 또는 _headers 앞) ──
HELPER = '''
    def _build_chat_context(self, messages, max_turns: int = 4, max_chars: int = 1500) -> str:
        """OpenWebUI __messages__ → Browser Agent용 짧은 대화 맥락 문자열."""
        if not messages:
            return ""
        filtered = []
        for m in messages:
            ...
            if len(content) > 400:
                content = content[:400] + "…"
            filtered.append(f"{role.upper()}: {content}")
        if not filtered:
            return ""
        # 최근 N턴 (대략 user+assistant 2개 = 1턴)
        recent = filtered[-(max(1, max_turns) * 2):]
        text = "\\n".join(recent)
        if len(text) > max_chars:
            text = text[-max_chars:]
        return text

    def _with_chat_context(self, task: str, messages) -> str:
        if not getattr(self.valves, "ENABLE_CHAT_CONTEXT", True):
            return task
        ctx = self._build_chat_context(
            messages,
            int(getattr(self.valves, "CONTEXT_TURNS", 4) or 4),
            int(getattr(self.valves, "MAX_CONTEXT_CHARS", 1500) or 1500),
        )
        if not ctx:
            return task
        combined = "[이전 대화 맥락]\\n" + ctx + "\\n\\n[현재 요청]\\n" + task
...
if "_build_chat_context" in code:
    ok_list.append("_build_chat_context 이미 존재")
else:
    inserted = False
    for anchor in (
        "    def _set_user(self, __user__):",
        "    def _headers(self) -> dict:",
    ):
        if anchor in code:
            code = code.replace(anchor, HELPER + anchor, 1)
            ok_list.append("_build_chat_context / _with_chat_context 추가")
            inserted = True
            break
    if not inserted:
        print("  ⚠️  헬퍼 삽입 앵커 없음")

# ── 3) browse 시그니처 + task에 맥락 주입 ──
# 5-3 패치 후 예상 형태
OLD_BROWSE_A = '''    async def browse(self, task: str, __event_emitter__=None, __user__={}):
        """Open a URL and perform a task. Do NOT use for weather/prices/stocks - use dedicated functions.
        :param task: URL + instruction
        """
        self._set_user(__user__)
        async def emit(msg, done=False):
            if __event_emitter__: await __event_emitter__({"type":"status","data":{"description":msg,"done":done}})
        if len(task.strip()) < 2: return "작업 내용이 너무 짧습니다."
        await emit("브라우저를 열고 작업 중...")
        try:
            result = await self._post("/browse", {"task": task, "max_steps": self.valves.DEFAULT_MAX_STEPS})'''

NEW_BROWSE_A = '''    async def browse(self, task: str, __event_emitter__=None, __user__={}, __messages__=None):
        """Open a URL and perform a task. Do NOT use for weather/prices/stocks - use dedicated functions.
        현재 OpenWebUI 채팅의 최근 대화를 함께 전달합니다.
        :param task: URL + instruction
        """
        self._set_user(__user__)
        task = self._with_chat_context(task, __messages__)
        async def emit(msg, done=False):
            if __event_emitter__: await __event_emitter__({"type":"status","data":{"description":msg,"done":done}})
        if len(task.strip()) < 2: return "작업 내용이 너무 짧습니다."
        await emit("브라우저를 열고 작업 중...")
...
OLD_BROWSE_B = '''    async def browse(self, task: str, __event_emitter__=None):
        """Open a URL and perform a task. Do NOT use for weather/prices/stocks - use dedicated functions.
        :param task: URL + instruction
        """
        async def emit(msg, done=False):
...
NEW_BROWSE_B = '''    async def browse(self, task: str, __event_emitter__=None, __user__={}, __messages__=None):
        """Open a URL and perform a task. Do NOT use for weather/prices/stocks - use dedicated functions.
        현재 OpenWebUI 채팅의 최근 대화를 함께 전달합니다.
        :param task: URL + instruction
        """
        if hasattr(self, "_set_user"):
            self._set_user(__user__)
        task = self._with_chat_context(task, __messages__)
        async def emit(msg, done=False):
            if __event_emitter__: await __event_emitter__({"type":"status","data":{"description":msg,"done":done}})
...
if "__messages__" in code and "_with_chat_context(task" in code:
    ok_list.append("browse 맥락 주입 이미 적용")
elif OLD_BROWSE_A in code:
    code = code.replace(OLD_BROWSE_A, NEW_BROWSE_A, 1)
    ok_list.append("browse: __messages__ + 맥락 주입 (5-3 이후 형태)")
elif OLD_BROWSE_B in code:
    code = code.replace(OLD_BROWSE_B, NEW_BROWSE_B, 1)
    ok_list.append("browse: __messages__ + 맥락 주입 (기본 형태)")
else:
    print("  ⚠️  browse 메서드 본문 불일치 — openwebui_tool.py를 직접 확인하세요")
...
ok "openwebui_tool.py 채팅 맥락 전달 패치 완료"
info "browse 호출 시 OpenWebUI __messages__ 최근 대화가 task에 포함됩니다"
