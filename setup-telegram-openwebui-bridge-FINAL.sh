...
if docker network ls --format '{{.Name}}' 2>/dev/null | grep -q "^openwebui_net$"; then
  ...
  print_info "openwebui_net 없음 — browser-agent 미설치 상태 (나중에 재실행하면 자동 연결)"
fi
...
async def health_server():
    ...
    async def metrics_auth(request):
        _int_secret = os.getenv("INTERNAL_API_SECRET", "")
        if _int_secret:
            if request.headers.get("Authorization", "") != f"Bearer {_int_secret}":
...
async def dashboard_server():
    ...
    def _check_auth(request) -> bool:
        ...
        provided = auth.replace("Bearer ", "").strip() or token.strip()
        if not provided:
            return False
...
<div id="toast"></div>
<script>
const TOKEN = new URLSearchParams(location.search).get('token') || localStorage.getItem('dt') || prompt('INTERNAL_API_SECRET 입력:');
