...
class Filter:
    ...
    def _get_memories(self, uid) -> List[str]:
        if not Memories or not uid:
            return []
        try:
            items = Memories.get_memories_by_user_id(uid) or []
            out = []
            for m in items:
    ...
    def _save_memory(self, uid, content):
        if not Memories or not uid or not content:
            return
        try:
            existing = self._get_memories(uid)
            if content.strip() in [e.strip() for e in existing]:
                return
            Memories.insert_new_memory(uid, content.strip())
        except Exception:
            pass
    ...
    async def inlet(self, body: dict, __user__: Optional[dict] = None, **kwargs) -> dict:
        ...
        mems = self._get_memories(uid)[: self.valves.max_inject]
        if not mems:
            return body
        try:
