...
chmod 700 "$BASE_DIR" "$BASE_DIR/secrets"
cd "$BASE_DIR"
...
NAVER_STATIC_MAP_URL = "https://naveropenapi.apigw.ntruss.com/map-static/v2/raster"
...
# 예: "openai/gpt-oss-120b"(빠름), "openai/gpt-oss-120b"(한글 정확)
    ...
    if _location_mission_requested(mission):
        ...
        if loc_state.get("consent") is True and not loc_state.get("reported") and _looks_like_location_answer(user_speech):
            ...
            if location:
                ...
                return Response(str(response), mimetype="text/xml")
            gather = Gather(input="speech dtmf", barge_in=True, numDigits=1, action=f"{SERVER_DOMAIN}/respond-out", language=tts_lang, timeout=TIMEOUT_OUTBOUND, speechTimeout=str(speech_to))
            gather.say("말씀하신 장소를 지도에서 정확히 찾지 못했어요. 가능하면 도로명 주소나 건물 이름과 함께 다시 말씀해 주세요.", voice=tts_voice, language=tts_lang)
            response.append(gather)
...
#   - "openai/gpt-oss-120b"  : 빠름. 일반 통화·대화에 적합. 가끔 한글이 살짝 깨질 수 있음
...
FAIL_CLOSED = os.getenv("EMBEDDING_MIGRATION_FAIL_CLOSED", "true").lower() == "true"
...
services:
  openapi-tools:
    ...
    environment:
      ...
      - EMBEDDING_MIGRATION_FAIL_CLOSED=${EMBEDDING_MIGRATION_FAIL_CLOSED:-true}
...
EMBEDDING_MIGRATION_FAIL_CLOSED=true
...
chmod 700 "$BASE_DIR/tools-api/data" "$BASE_DIR/twilio-bot/data" 2>/dev/null || true
if [ -d "$BASE_DIR/tools-api/data" ]; then
  find "$BASE_DIR/tools-api/data" -type f \( -name 'embedding_registry.json' -o -name 'embedding_registry.bak' -o -name 'embedding_registry.lock' -o -name 'contacts.json' -o -name 'call_memories.json' \) -exec chmod 600 {} + 2>/dev/null || true
fi
...
# ============================================================================
# FINAL INSTALLATION VALIDATION / HARDENING
# ============================================================================
set -Eeuo pipefail
cd "$BASE_DIR"
umask 077
...
chmod 600 speaker-ai.env 2>/dev/null || true
chmod 600 embedding-ai.env 2>/dev/null || true
chmod 600 ./secrets/naver_maps_client_secret 2>/dev/null || true

# Compose must be valid before continuing.
if ! docker compose config >/tmp/hackyhappy-final-compose.yml 2>/tmp/hackyhappy-final-compose-error.log; then
  echo "❌ 최종 Docker Compose 검증 실패"
  cat /tmp/hackyhappy-final-compose-error.log
  exit 1
fi

echo "✅ Docker Compose 구성 OK"

# Required services.
REQUIRED_SERVICES="qdrant openapi-tools twilio-bot browser-agent redis open-webui realtime-voice"
SERVICES="$(docker compose config --services)"
for svc in $REQUIRED_SERVICES; do
  printf '%s\n' "$SERVICES" | grep -qx "$svc" || { echo "❌ 필수 서비스 누락: $svc"; exit 1; }
done
echo "✅ 필수 서비스 7개 확인"

# NAVER location feature.
if ! grep -q '^NAVER_LOCATION_ENABLED=' .env 2>/dev/null; then
  echo 'NAVER_LOCATION_ENABLED=false' >> .env
fi
if ! grep -q '^NAVER_MAPS_CLIENT_ID=' .env 2>/dev/null; then
  echo 'NAVER_MAPS_CLIENT_ID=' >> .env
fi
mkdir -p ./secrets
: > /dev/null
[ -f ./secrets/naver_maps_client_secret ] || touch ./secrets/naver_maps_client_secret
chmod 700 ./secrets
chmod 600 ./secrets/naver_maps_client_secret

for symbol in _location_mission_requested _naver_geocode send_location_to_telegram; do
  grep -q "$symbol" ./twilio-bot/twilio_bot.py || { echo "❌ 위치 기능 누락: $symbol"; exit 1; }
done
echo "✅ NAVER 위치 코드 확인"

# Local Python syntax check.
python3 -m py_compile ./twilio-bot/twilio_bot.py
echo "✅ twilio_bot.py 로컬 문법 OK"

# Start/reconcile services from the correct directory.
...
# Container-side Python syntax and NAVER secret mount.
docker compose exec -T twilio-bot python3 -m py_compile /app/twilio_bot.py
echo "✅ twilio_bot.py 컨테이너 문법 OK"

docker compose exec -T twilio-bot sh -c '[ -f /run/secrets/naver_maps_client_secret ]'
echo "✅ NAVER Secret mount OK"

# Twilio health.
HEALTH_OK=false
for _i in $(seq 1 30); do
  if docker compose exec -T twilio-bot sh -c 'curl -fsS http://127.0.0.1:5000/health >/dev/null' 2>/dev/null; then
    HEALTH_OK=true
    break
  fi
  sleep 2
...
if [ "$HEALTH_OK" != true ]; then
  echo "❌ twilio-bot /health 실패"
  docker compose logs --tail=100 twilio-bot || true
  exit 1
fi
echo "✅ twilio-bot /health OK"

# Final secret permissions.
[ "$(stat -c '%a' .env)" = "600" ] || { echo "❌ .env 권한 오류"; exit 1; }
[ "$(stat -c '%a' ./secrets/naver_maps_client_secret)" = "600" ] || { echo "❌ NAVER Secret 권한 오류"; exit 1; }

echo
echo "======================================================"
echo " ✅ HackyHappy FINAL 설치 완료 / 검증 완료"
echo "======================================================"
echo "📁 $BASE_DIR"
echo "📍 NAVER_LOCATION_ENABLED=$(grep '^NAVER_LOCATION_ENABLED=' .env | tail -1 | cut -d= -f2-)"
echo "🔒 .env 권한: 600"
echo "🔒 NAVER Secret 권한: 600"
echo "🐳 Docker 서비스 상태:"
docker compose ps
echo
echo "GitHub 업로드 전 검사:"
echo "  bash -n $HOME/$(basename "$0")"
echo "  grep -nE 'V2=|bash \"\\$V2\"' $HOME/$(basename "$0")"
echo "======================================================"
