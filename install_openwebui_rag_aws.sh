#!/bin/bash
# =============================================================================
# 프로젝트명: OpenWebUI RAG AWS 보안강화 원터치 설치 스크립트
# 제작일: 2026-01-26
# 설명: AWS EC2 + Docker + Ollama + Groq + Qdrant + Nginx + SSL + UFW + 보안강화
# 라이센스: MIT License
#
# ✅ 지원 OS : Ubuntu 20.04 / 22.04 / 24.04 LTS
# ✅ 권장 인스턴스 : t3.medium(4GB↑) / t3.large(8GB↑) / t3.xlarge(16GB↑)
#
# 📦 원격 원터치 설치:
...
BANNER
echo -e "${NC}"
echo -e "  ${BOLD}🔐 AWS 보안강화 RAG 원터치 설치 스크립트${NC}"
...
if [ -n "$IMDS_TOKEN" ]; then
  IS_AWS=true
  AWS_INSTANCE_TYPE=$(curl -sf --connect-timeout 2 \
  ...
  AWS_PUBLIC_IP=$(curl -sf --connect-timeout 2 \
    -H "X-aws-ec2-metadata-token: $IMDS_TOKEN" \
...
# 외부 IP 폴백
[ "$AWS_PUBLIC_IP" = "Unknown" ] && \
  AWS_PUBLIC_IP=$(curl -sf --connect-timeout 3 https://api.ipify.org 2>/dev/null || \
                  curl -sf --connect-timeout 3 https://ifconfig.me 2>/dev/null || \
                  echo "Unknown")
...
info "AWS EC2     : $([ "$IS_AWS" = true ] && echo "$AWS_INSTANCE_TYPE ($AWS_REGION)" || echo "비 AWS 또는 감지 불가")"
