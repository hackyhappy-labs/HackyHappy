...
# (4) 캘린더 데드락 방지: OpenWebUI 멀티 워커 확인
#     단일 워커면 채팅 캘린더 도구가 self-call로 15초 타임아웃(데드락) 발생
OWUI_CONTAINER=$(docker ps --format '{{.Names}}' 2>/dev/null | grep -E "open-webui" | head -1)
if [ -n "$OWUI_CONTAINER" ]; then
  WORKERS=$(docker exec "$OWUI_CONTAINER" sh -c 'cat /proc/1/environ 2>/dev/null | tr "\0" "\n" | grep "^UVICORN_WORKERS=" | cut -d= -f2' 2>/dev/null)
  if [ -n "$WORKERS" ] && [ "$WORKERS" -ge 2 ] 2>/dev/null; then
    ok "OpenWebUI 멀티 워커 적용됨 (${WORKERS}개) — 캘린더 self-call 데드락 방지"
  else
    warn "OpenWebUI 단일 워커 — 채팅 캘린더가 15초 타임아웃될 수 있음"
    info "  해결: docker-compose.yml open-webui에 UVICORN_WORKERS=4 추가 후 재생성"
  fi
fi
