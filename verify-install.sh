...
# ║             또는 start-openwebui-VoiceMem-integrated.sh (고객상담용)   ║
# ║  - Phase 3: setup-telegram-bridge-calendar.sh                      ║
# ║  - Browser: setup-browser-agent-calendar-naver-search-only.sh      ║
# ║                                                                      ║
# ║  v6.1.0 변경사항:                                                   ║
# ║  - [갱신] 스크립트 파일명 최신화                                     ║
# ║          customer-support → VoiceMem-integrated                     ║
# ║          setup-browser-agent-calendar → -naver-search-only          ║
# ║  - [추가] 섹션 12-6 VoiceMem 이중뇌 검증:                            ║
# ║          VOICEMEM_ENABLED 스위치 · 엔진(ollama|openai) 판별          ║
# ║          OpenAI 엔진 시 임베딩/분석 모델·OPENAI_API_KEY 존재 확인    ║
# ║  - [교정] Ollama 접근 실패 시 VoiceMem 엔진=openai 면 경고→정보로    ║
# ║          (OpenAI 임베딩 사용 시 Ollama 불필요이므로 오탐 방지)       ║
# ║                                                                      ║
# ║  v6.0.0 변경사항:                                                   ║
# ║  - [갱신] 스크립트 파일명 최신화 (admin-only/VoiceMem-integrated 등)   ║
# ║  - [추가] 캘린더 연동 검증 (owui-data 마운트·shared-key·도구 등록)  ║
# ║  - [추가] 보안 강화 검증 (requests/urllib3 CVE 패치 버전)           ║
# ║  - [변경] PIN 인증 검증 → 관리자 번호(ADMIN_NUMBERS) + PIN 게이트   ║
# ║  - [갱신] 관리자 명령 PIN 인증 재도입 감지 (ADMIN_PIN_REQUIRED)      ║
# ║           고객상담용=활성(6자리), 개인용=번호기반 — 둘 다 정상 인정  ║
# ║  - [추가] 캘린더 등록/특정날짜 조회 검증 (create_event/get_schedule) ║
# ║  - [추가] 끼어들기 정확도 개선 검증 (BARGEIN_CPS)                    ║
...
declare -a DIR_LIST=(
  # ── Phase 2: start-openwebui-hardened-admin-only.sh / VoiceMem-integrated.sh ──
  ...
  "Phase2|realtime-voice (실시간 음성, 선택)|${OWUI_DIR}/realtime-voice"
  ...
  # ── Browser Agent: setup-browser-agent-calendar-naver-search-only.sh ──
...
declare -a FILE_LIST=(
  ...
  # ── Phase 2: realtime-voice (실시간 음성 서버, REALTIME_MODE 사용 시) ──
  "Phase2|realtime-voice/realtime_server.py|${OWUI_DIR}/realtime-voice/realtime_server.py"
  "Phase2|realtime-voice/Dockerfile|${OWUI_DIR}/realtime-voice/Dockerfile"
  "Phase2|realtime-voice/requirements.txt|${OWUI_DIR}/realtime-voice/requirements.txt"
...
if ! command -v docker &>/dev/null; then
  ...
  for CNAME in \
    ...
    "realtime-voice|Phase2|🎙️ 실시간 음성 서버 (선택)" \
...
# ── Phase 2: 🎙️ realtime-voice (:5001, REALTIME_MODE=true 일 때만) ──
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "realtime-voice"; then
  RT_RUN=$(docker inspect --format='{{.State.Running}}' realtime-voice 2>/dev/null || echo "false")
  if [ "$RT_RUN" = "true" ]; then
    ok "[Phase2] 🎙️ realtime-voice (:5001): running (실시간 모드 활성)"
  else
    warn "[Phase2] 🎙️ realtime-voice: 컨테이너 존재하나 미실행"
    info "  → docker logs realtime-voice --tail 20 으로 원인 확인"
  fi
else
  info "[Phase2] 🎙️ realtime-voice: 미실행 (REALTIME_MODE=false, 선택 기능)"
fi
...
if [ -f "${OWUI_DIR}/.env" ]; then
  ...
  # 통화 인증: 관리자 번호(ADMIN_NUMBERS) + (선택) 관리자 명령 PIN 확인
  ADM_NUM=$(grep "^ADMIN_NUMBERS=" "${OWUI_DIR}/.env" 2>/dev/null | head -1 | cut -d= -f2-)
  if [ -n "$ADM_NUM" ]; then
    NUM_COUNT=$(echo "$ADM_NUM" | tr ',' '\n' | grep -c .)
    ok "관리자 번호 설정됨 (${NUM_COUNT}개) — 이 번호만 봇에게 명령 가능"
  else
    warn "ADMIN_NUMBERS 미설정 — 통화 인증 대상 없음. 설치 시 본인 번호 입력 확인 필요"
  fi
  # 관리자 명령 PIN 게이트 활성 여부 감지 (고객상담용은 켜짐, 개인용은 보통 미적용)
  if [ -f "${OWUI_DIR}/twilio-bot/twilio_bot.py" ]; then
    if grep -q "ADMIN_PIN_REQUIRED" "${OWUI_DIR}/twilio-bot/twilio_bot.py" 2>/dev/null \
       && grep -q "_is_sensitive_command" "${OWUI_DIR}/twilio-bot/twilio_bot.py" 2>/dev/null; then
      PIN_CFG=$(grep "^ADMIN_PIN_REQUIRED" "${OWUI_DIR}/twilio-bot/ai_config.py" 2>/dev/null | head -1 | cut -d= -f2- | tr -d ' ')
      if [ "$PIN_CFG" = "True" ]; then
        ok "관리자 명령 PIN 인증 활성화됨 — 민감 명령(저장·전화·문자·일정등록) 실행 시 PIN 확인"
        # PIN 값이 설치 시 저장됐는지 확인
        if [ -s "${OWUI_DIR}/secrets/admin_pin" ]; then
          PIN_LEN=$(tr -d '\n' < "${OWUI_DIR}/secrets/admin_pin" | wc -c)
          if [ "$PIN_LEN" -eq 6 ]; then
            ok "  PIN 6자리 저장됨 (secrets/admin_pin)"
          else
            warn "  PIN 길이 이상(${PIN_LEN}자) — 설치 시 6자리 입력 확인 필요"
          fi
        else
          warn "  secrets/admin_pin 비어있음 — PIN 인증이 정상 동작하지 않을 수 있음"
        fi
      else
        info "관리자 명령 PIN 인증 비활성화(ADMIN_PIN_REQUIRED=False) — 번호만으로 명령 실행"
      fi
    else
      info "관리자 명령 PIN 게이트 미적용 (개인용/구버전) — 등록된 관리자 번호 기반 인증"
    fi
  fi
  ...
  if [ -f "${OWUI_DIR}/twilio-bot/twilio_bot.py" ]; then
    ...
    # 끼어들기(barge-in) 정확도 개선 적용 여부
    if grep -q "BARGEIN_CPS" "${OWUI_DIR}/twilio-bot/twilio_bot.py" 2>/dev/null; then
      ok "끼어들기 정확도 개선 적용됨 (언어별 실측 낭독시간 + 신뢰도 보조 판단)"
    else
      info "끼어들기 정확도 개선 미적용(구버전) — 글자수 기반 추정 사용 중"
    fi
  fi
  ...
  if [ -n "$OWUI_KEY" ] && [ ${#OWUI_KEY} -ge 20 ]; then
    ...
    if echo "$TOOL_CHECK" | grep -q '"id":"ai_browser_agent"'; then
      ...
      if [ -n "$TOOL_CONTENT" ]; then
        ...
        echo "$TOOL_CONTENT" | grep -q "get_today_schedule" && ok "    get_today_schedule() 존재 (캘린더 오늘조회)" || info "    get_today_schedule() 없음 — 캘린더 패치 미적용"
        echo "$TOOL_CONTENT" | grep -q "get_schedule\b\|get_schedule(" && ok "    get_schedule() 존재 (캘린더 특정날짜/기간 조회)" || info "    get_schedule() 없음 — 특정날짜 조회 미적용"
        echo "$TOOL_CONTENT" | grep -q "create_event" && ok "    create_event() 존재 (캘린더 일정 등록)" || info "    create_event() 없음 — 일정 등록 미적용"
    ...
    # Phase 2 Tool 확인 (12개: 기존 9 + 기억의 앵커·귓속말·음성엔진)
    for TOOL_ID in \
      ...
      "calendar_today|캘린더 (조회·등록)" \
      "memory_anchor|🧠 기억의 앵커 (과거 통화 회상)" \
      "live_whisper|🤫 실시간 귓속말 (대리통화 지시)" \
      "voice_engine_config|🔧 음성엔진 설정 (실시간 STT/TTS)"; do
      ...
      if echo "$TC" | grep -q "\"id\":\"${TID}\""; then
        ...
        # 캘린더 도구면 조회·등록 함수까지 확인
        if [ "$TID" = "calendar_today" ]; then
          CAL_CONTENT=$(echo "$TC" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('content',''))" 2>/dev/null || echo "")
          if [ -n "$CAL_CONTENT" ]; then
            echo "$CAL_CONTENT" | grep -q "get_today_schedule"   && ok "    ├ 오늘 일정 조회 지원"          || info "    ├ 오늘 조회 함수 없음"
            echo "$CAL_CONTENT" | grep -q "get_schedule_by_date" && ok "    ├ 특정 날짜 조회 지원"          || info "    ├ 특정 날짜 조회 미적용(구버전)"
            echo "$CAL_CONTENT" | grep -q "create_event"         && ok "    └ 일정 등록 지원"              || info "    └ 일정 등록 미적용(구버전)"
          fi
        fi
      else
...
# ════════════════════════════════
# 12-6. 🆕 신규 기능 검증 (기억의 앵커·실시간·귓속말·Ollama)
# ════════════════════════════════
section "12-6. 신규 기능 검증 (기억의 앵커·VoiceMem 이중뇌·실시간·귓속말)"

# ── 🧠 기억의 앵커: twilio_bot.py 함수 존재 확인 ──
BOT_PY="${OWUI_DIR}/twilio-bot/twilio_bot.py"
if [ -f "$BOT_PY" ]; then
  grep -q "def save_call_memory" "$BOT_PY"   && ok "🧠 기억의 앵커: 저장 함수(save_call_memory) 존재" || warn "🧠 기억의 앵커: save_call_memory 없음"
  grep -q "def recall_memories" "$BOT_PY"     && ok "🧠 기억의 앵커: 회상 함수(recall_memories) 존재" || warn "🧠 기억의 앵커: recall_memories 없음"
  grep -q "def _memory_autocall_loop" "$BOT_PY" && ok "🧠 기억의 앵커: 능동형 자동전화 루프 존재" || warn "🧠 자동전화 루프 없음"
  grep -q '@app.route("/memory/save"' "$BOT_PY" && ok "🧠 기억의 앵커: HTTP 엔드포인트(/memory/save,recall) 존재" || warn "🧠 memory 엔드포인트 없음"
  # 🤫 귓속말
  grep -q "def _telegram_whisper_poller" "$BOT_PY" && ok "🤫 실시간 귓속말: 텔레그램 폴러 존재" || warn "🤫 귓속말 폴러 없음"
  grep -q '@app.route("/whisper/send"' "$BOT_PY"    && ok "🤫 실시간 귓속말: HTTP 엔드포인트(/whisper/*) 존재" || warn "🤫 whisper 엔드포인트 없음"
  # 🎙️ 실시간
  grep -q "def build_stream_twiml" "$BOT_PY"    && ok "🎙️ 실시간: build_stream_twiml (Media Stream) 존재" || warn "🎙️ build_stream_twiml 없음"
  grep -q "REALTIME_MODE" "$BOT_PY"             && ok "🎙️ 실시간: REALTIME_MODE 분기 존재" || warn "🎙️ REALTIME_MODE 없음"
  # SMS 250자
  grep -q "250자 이내" "$BOT_PY"                && ok "📱 SMS 보고: 250자 확대 적용됨" || info "📱 SMS 250자 미적용(구버전)"
else
  info "twilio_bot.py 없음 — 신규 기능 검증 건너뜀 (Phase 2 미설치)"
fi

# ── ai_config.py: 자동전화 스위치 확인 ──
AIC_PY="${OWUI_DIR}/twilio-bot/ai_config.py"
if [ -f "$AIC_PY" ]; then
  if grep -q "MEMORY_AUTOCALL_ENABLED" "$AIC_PY"; then
    AC_STATE=$(grep "MEMORY_AUTOCALL_ENABLED" "$AIC_PY" | head -1 | grep -oiE "True|False" | head -1)
    AC_MODE=$(grep "MEMORY_AUTOCALL_MODE" "$AIC_PY" | head -1 | grep -oE '"[a-z]+"' | head -1)
    ok "🧠 자동전화 스위치: MEMORY_AUTOCALL_ENABLED=${AC_STATE:-?}, MODE=${AC_MODE:-?}"
    if [ "${AC_STATE}" = "True" ] && [ "${AC_MODE}" = '"auto"' ]; then
      warn "  ⚠️ 자동전화가 'auto'로 켜져 있습니다 — 요금 폭탄 주의! (suggest 권장)"
    fi
  else
    info "🧠 자동전화 스위치 미적용 (구버전 ai_config.py)"
  fi
fi

# ── 🧠🧠 VoiceMem 이중뇌 검증 (좌/우뇌 · 엔진 Ollama|OpenAI 택1) ──
AIC_PY="${OWUI_DIR}/twilio-bot/ai_config.py"
VOICEMEM_ON=""; VOICEMEM_ENGINE=""
if [ -f "$AIC_PY" ]; then
  if grep -q "VOICEMEM_ENABLED" "$AIC_PY" 2>/dev/null; then
    VOICEMEM_ON=$(grep "VOICEMEM_ENABLED" "$AIC_PY" | head -1 | grep -oiE "True|False" | head -1)
    ok "🧠🧠 VoiceMem 스위치: VOICEMEM_ENABLED=${VOICEMEM_ON:-?}"
    if [ "${VOICEMEM_ON}" = "True" ]; then
      # 엔진 확인 (ollama 기본 / openai 선택)
      VOICEMEM_ENGINE=$(grep "VOICEMEM_ENGINE" "$AIC_PY" | head -1 | grep -oE '"[a-z]+"' | head -1 | tr -d '"')
      case "${VOICEMEM_ENGINE}" in
        openai)
          ok "  ├ 엔진: OpenAI API (클라우드 임베딩·분석)"
          # 임베딩/분석 모델 표기 확인
          EMB_M=$(grep "VOICEMEM_OPENAI_EMBED_MODEL" "$AIC_PY" | head -1 | grep -oE '"[^"]+"' | head -1 | tr -d '"')
          CHT_M=$(grep "VOICEMEM_OPENAI_CHAT_MODEL"  "$AIC_PY" | head -1 | grep -oE '"[^"]+"' | head -1 | tr -d '"')
          [ -n "$EMB_M" ] && ok "  ├ 임베딩 모델: ${EMB_M}" || info "  ├ 임베딩 모델 미표기 (기본값 사용 가능)"
          [ -n "$CHT_M" ] && ok "  ├ 분석 모델: ${CHT_M}"   || info "  ├ 분석 모델 미표기 (기본값 사용 가능)"
          # OpenAI 키 존재 확인 (.env 또는 Docker Secrets)
          OAI_OK=no
          if grep -q "^OPENAI_API_KEY=." "${OWUI_DIR}/.env" 2>/dev/null; then OAI_OK=yes; fi
          if [ -f "${OWUI_DIR}/secrets/openai_api_key" ] || sudo test -f "${OWUI_DIR}/secrets/openai_api_key" 2>/dev/null; then
            OAI_LEN=$(sudo cat "${OWUI_DIR}/secrets/openai_api_key" 2>/dev/null | tr -d '\n' | wc -c 2>/dev/null || echo 0)
            [ "${OAI_LEN:-0}" -ge 20 ] && OAI_OK=yes
          fi
          if [ "$OAI_OK" = yes ]; then
            ok "  └ OpenAI API 키 설정됨 (.env 또는 secrets) — VoiceMem 임베딩·분석 준비됨"
          else
            fail "  └ OpenAI 엔진인데 OPENAI_API_KEY 없음 — VoiceMem 임베딩·분석 실패. .env 또는 secrets/openai_api_key 설정 필요"
          fi
          ;;
        ollama|"")
          ok "  └ 엔진: Ollama (로컬 임베딩·분석) — 아래 Ollama 바인딩 확인 참조"
          VOICEMEM_ENGINE="ollama"
          ;;
        *)
          warn "  └ 엔진 값이 예상 밖(${VOICEMEM_ENGINE}) — 'ollama' 또는 'openai' 여야 함"
          ;;
      esac
    else
      info "🧠🧠 VoiceMem 비활성 (VOICEMEM_ENABLED=False) — 이중뇌 기억 저장·회상 꺼짐 (선택)"
    fi
  else
    info "🧠🧠 VoiceMem 미적용 (구버전 ai_config.py — VOICEMEM_ENABLED 없음)"
  fi
fi

# ── 🦙 Ollama 바인딩 확인 (기억의 앵커 임베딩용) ──
if [ -f "${OWUI_DIR}/.env" ]; then
  OLLAMA_URL=$(grep "^OLLAMA_BASE_URL" "${OWUI_DIR}/.env" 2>/dev/null | head -1 | cut -d= -f2-)
  if [ -n "$OLLAMA_URL" ]; then
    # 컨테이너에서 실제 접근되는지 (0.0.0.0 바인딩 확인)
    if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "twilio-bot"; then
      OLLAMA_TEST=$(docker exec twilio-bot python3 -c "
import os,urllib.request
try:
    u=os.getenv('OLLAMA_BASE_URL','')
    urllib.request.urlopen(u+'/api/tags',timeout=5); print('OK')
except Exception as e: print('FAIL')
" 2>/dev/null || echo "FAIL")
      if [ "$OLLAMA_TEST" = "OK" ]; then
        ok "🦙 Ollama 접근 확인됨 (${OLLAMA_URL}) — 기억의 앵커 임베딩 정상"
      elif [ "${VOICEMEM_ENGINE}" = "openai" ]; then
        info "🦙 컨테이너→Ollama 접근 실패 — 단, VoiceMem 엔진이 OpenAI라 영향 없음 (임베딩은 OpenAI 사용)"
      else
        warn "🦙 컨테이너→Ollama 접근 실패 — 기억의 앵커가 더미 벡터로 폴백될 수 있음"
        info "  → WSL2 안에서 'OLLAMA_HOST=0.0.0.0:11434 ollama serve' 로 실행하세요"
        info "  → 또는 VoiceMem 엔진을 OpenAI로 전환하면 Ollama 없이 동작합니다 (ai_config.py: VOICEMEM_ENGINE=\"openai\")"
      fi
    else
      info "🦙 Ollama URL 설정됨(${OLLAMA_URL}) — twilio-bot 미실행이라 접근 테스트 건너뜀"
    fi
  else
    info "🦙 OLLAMA_BASE_URL 미설정 (Ollama 미사용)"
  fi
fi

# ── 📨 텔레그램 보고 / 🎙️ 실시간 .env 설정 확인 ──
if [ -f "${OWUI_DIR}/.env" ]; then
  if grep -q "^TELEGRAM_BOT_TOKEN=." "${OWUI_DIR}/.env" 2>/dev/null; then
    ok "📨 텔레그램 보고: 봇 토큰 설정됨"
  else
    info "📨 텔레그램 보고: 미설정 (문자 보고만 옴, 선택)"
  fi
  RT_MODE=$(grep "^REALTIME_MODE" "${OWUI_DIR}/.env" 2>/dev/null | head -1 | cut -d= -f2-)
  if [ "$RT_MODE" = "true" ]; then
    ok "🎙️ 실시간 모드: 활성 (REALTIME_MODE=true)"
    grep -q "^DEEPGRAM_API_KEY=." "${OWUI_DIR}/.env"   && ok "  ├ Deepgram 키 설정됨"   || warn "  ├ Deepgram 키 없음 — 실시간 STT 불가"
    grep -q "^ELEVENLABS_API_KEY=." "${OWUI_DIR}/.env" && ok "  └ ElevenLabs 키 설정됨" || warn "  └ ElevenLabs 키 없음 — 실시간 TTS 불가"
  else
    info "🎙️ 실시간 모드: 비활성 (기존 방식, 선택 기능)"
  fi
fi
...
info "                     bash setup-browser-agent-calendar-naver-search-only.sh"
