#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════╗
# ║  설치 검증 스크립트 — verify-install.sh  v5.2.0                     ║
# ║  3개 설치 스크립트 전체 대조 검증 (실제 소스코드 기준 정합성 수정)  ║
# ║                                                                      ║
# ║  검증 대상:                                                          ║
...
cat << 'BANNER'
╔══════════════════════════════════════════════════════════════════════╗
║  OpenWebUI AI 에이전트 — 전체 설치 검증 v5.2.0                      ║
║  Phase 2 + Phase 3 + Browser Agent v6.4 완전 대조 (섹션 15개)       ║
...
declare -a DIR_LIST=(
  ...
  # ── Phase 3: setup-telegram-openwebui-bridge-FINAL.sh ──
...
" 2>/dev/null || echo ""
  ...
  if echo "$MHLT" | grep -q '"status"\|"multi_agent"\|"graph"'; then
    ok "  /health/multi: 응답 OK (Multi-Agent 그래프 로드됨)"
