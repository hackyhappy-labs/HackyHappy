#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════╗
# ║  설치 검증 스크립트 — verify-install.sh  v5.0.0                     ║
# ║  3개 설치 스크립트 전체 대조 검증                                    ║
# ║                                                                      ║
# ║  검증 대상:                                                          ║
# ║  - Phase 2: start-openwebui-hardened.sh (v1.1.0)                   ║
# ║  - Phase 3: setup-telegram-openwebui-bridge-FINAL.sh (v2.0.0)      ║
# ║  - Browser: setup-browser-agent-browser-use-v6.sh (v6.4.0)         ║
# ║                                                                      ║
# ║  v5.0.0 변경사항 (v4.0.0 대비):                                    ║
# ║  - Browser Agent 신규 엔드포인트 9개 검증 추가                      ║
# ║  - Browser Agent 신규 Tool 6개 검증 추가                            ║
# ║  - Browser Agent 보안 7항목 패치 적용 여부 확인                     ║
# ║  - Browser Agent 신규 패키지 확인 (sse-starlette/openpyxl/httpx)   ║
# ║  - Telegram 브릿지 스트리밍/예약/대시보드/get_available_models 확인 ║
# ║  - Telegram 보안 26항목 적용 여부 확인                              ║
# ║  - 섹션 14 신규: Browser Agent 기능 종합 검증                       ║
...
cat << 'BANNER'
╔══════════════════════════════════════════════════════════════════════╗
║  OpenWebUI AI 에이전트 — 전체 설치 검증 v5.0.0                      ║
║  Phase 2 + Phase 3 + Browser Agent v6.4 완전 대조 (섹션 14개)       ║
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "telegram-openwebui-bridge"; then
  ...
  TG_CODE=$(docker exec telegram-openwebui-bridge cat /app/bot/telegram_bot.py 2>/dev/null || echo "")
  if [ -n "$TG_CODE" ]; then
    echo "$TG_CODE" | grep -q "seen_update_ids\|update_id"   && ok "  ⑤ Replay Attack 방어됨"        || warn "  ⑤ Replay Attack 방어 미적용"
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "browser-agent"; then
  ...
  # 신규 패키지 확인 (v6.4 업그레이드)
  info "신규 패키지 확인 (v6.4 업그레이드):"
...
# ════════════════════════════════
# 14. Browser Agent 기능 종합 검증
# ════════════════════════════════
section "13. Browser Agent 기능 종합 검증 (v6.4)"
...
try:
    ...
    return print(r.read().decode()[:200])
...
" 2>/dev/null || echo ""
  ...
  info "browser-agent 미실행 — 섹션 14 건너뜀"
fi
...
info "                     bash setup-browser-agent-browser-use-v6.sh"
...
info "                     bash setup-telegram-openwebui-bridge-FINAL.sh"
