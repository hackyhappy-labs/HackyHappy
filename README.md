...
> ⚠️ **Version note (please read first):** This package by HappyHackyLab is **built and kept current against OpenWebUI 0.9.6**. If you need a different (newer) release, visit the [OpenWebUI official site / GitHub](https://github.com/open-webui/open-webui) to upgrade it yourself or check that version's compatibility. On higher versions some UI/API details may differ, which can affect tool registration or calendar behavior.
...
- [🧠🧠 VoiceMem Dual-Brain (Left/Right Memory)](#-voicemem-dual-brain-leftright-memory)
- [📍 Location Check (NAVER + Kakao + Google)](#-location-check-naver--kakao--google)
- [🛰️ GPS Location Share (elder care)](#️-gps-location-share-elder-care)
...
| 🧠🧠 VoiceMem Dual-Brain | The memory engine behind Memory Anchor. Splits each call memory into a **left brain (facts/structure: who·when·what)** and a **right brain (emotion/context: mood·association)** for storage and recall. An optional component you can turn on/off, and you pick **Ollama (local) or the OpenAI API** for its embeddings and analysis |
| 📍 Location Check (NAVER + Kakao + Google) | During a check-in call, **after obtaining the person's consent**, asks for their location and reports it to **Telegram with a map image**. NAVER Geocoding (road/lot) + Kakao Local (station/business/landmark names) resolve coordinates; the map image is a **Google hybrid satellite** photo (NAVER map if no Google key). Does not secretly collect phone GPS |
| 🛰️ GPS Location Share (elder care) | Sends a **location-share link by call + SMS**. When the person opens it and **taps [Allow] to consent**, their live GPS location is sent to the admin's Telegram and phone. Works around the "no GPS over a call" limit via a link + browser location permission. 5-layer security (random token, one-time, time limit, HTTPS, target-number binding) |
...
`start-openwebui-VoiceMem-integrated.sh` asks for the use case **as the very first thing** when you run it.
...
- (Optional) For VoiceMem dual-brain memory — **Ollama (local, free)** or an **OpenAI API key** (pick one). Choosing OpenAI runs embeddings and analysis in the cloud with no Ollama install required
...
```bash
wget https://YOUR-HOST/start-openwebui-VoiceMem-integrated.sh
less start-openwebui-VoiceMem-integrated.sh    # review before running (recommended)
chmod +x start-openwebui-VoiceMem-integrated.sh
./start-openwebui-VoiceMem-integrated.sh
```
...
| NAVER location check | `y` / `N` | `y` prompts for NAVER Maps Client ID·Secret → enables location check + map reporting. Then (optional) a Kakao REST API key (station/business names) and (optional) a Google Maps API key (satellite map). `N` (default) = location feature off |
...
> **📍 Location check is also optional.** Answer `y` at "Enable NAVER Maps location check? (y/N)" to enter your NAVER Maps keys and turn on the location feature. Optionally add a Kakao REST API key to also resolve station/business names. Location reports arrive via **Telegram** with a map image, so enabling Telegram reports together is recommended. See [Location Check (NAVER + Kakao + Google)](#-location-check-naver--kakao--google) for details.

When installation finishes, the full stack (OpenWebUI, phone bot, RAG, realtime voice server, etc.) starts automatically and **14 tools (including calendar, live whisper, voice engine settings, memory anchor, NAVER location check, and GPS location share) are registered automatically**.

### Step 2 — Browser agent (optional)

```bash
chmod +x setup-browser-agent-calendar-naver-search-api-only.sh
./setup-browser-agent-calendar-naver-search-api-only.sh
```
...
**How to find the OPENWEBUI_API_KEY value:**

The key in step 3 is normally generated in the web UI (**Settings -> Account -> API Keys**), but if the installer auto-generated one, it may already be in `.env` and you can read it from the server:

```bash
# Read the OpenWebUI API key from .env (paste this into the valve)
grep OPENWEBUI_API_KEY ~/OpenWebUI/.env
```

Example output:
```
OPENWEBUI_API_KEY=eyJhbGciOi...   (JWT token)  or  sk-...
```

Copy the value after `=` and paste it into the calendar valve's **`OPENWEBUI_API_KEY`** field, then Save. (Put the same value in the browser-agent tool valve too.)

> If this value is missing or empty in `.env`, it hasn't been issued yet — generate one in the web UI per step 3. If the issued key is `eyJ...` (a JWT), it expires; for long-term use prefer the `sk-...` format (see [API key format](#api-key-format)).
...
### Find the OpenWebUI API key on the server (`OPENWEBUI_API_KEY`)

This is the key you paste into the `OPENWEBUI_API_KEY` field of the calendar and browser tool valves. The normal way is to generate it in the web UI (**Settings -> Account -> API Keys**), but if the installer auto-generated one, it may already be in `.env` and you can read it straight from the server:

```bash
# Read the OpenWebUI API key from .env
grep OPENWEBUI_API_KEY ~/OpenWebUI/.env
```
...
Copy the value after `=` and paste it into **Workspace -> Tools -> Calendar / AI Browser Agent -> Valves -> `OPENWEBUI_API_KEY`**, then Save. Put the **same value** in both tools.

> If this value is missing or empty in `.env`, it hasn't been issued yet — generate one in the web UI (**Settings -> Account -> API Keys**) and paste it into both tool valves. If the issued key is `eyJ...` (a JWT), it expires; for long-term use prefer the `sk-...` format (see [API key format](#api-key-format)).
...
## 🧠🧠 VoiceMem Dual-Brain (Left/Right Memory)

**VoiceMem** is the memory engine behind [Memory Anchor](#-memory-anchor-call-recall--proactive-auto-call). Just as the human brain divides work between its hemispheres, VoiceMem stores and recalls a single call memory along **two tracks**. It is an **optional component** — turn it off and everything else keeps working.

### Why a dual brain

Storing memory as one blob mixes "exact facts" with "the mood of the moment," so recall tends to lose one or the other. VoiceMem separates the two and stores each the way it's best handled.

| Track | Brain | What it stores | How it's recalled |
|-------|-------|----------------|-------------------|
| **Left** | Facts·Structure | **Who·when·what** — phone number, name, date, time-of-day, topic, appointments, requests, and other structured facts | Looked up by exact keys (number·date·topic) |
| **Right** | Emotion·Context | **Mood·emotion·association** — emotional state, conversational tone, weather, the feel of the time-of-day | Semantic embedding — similarity search for "a mood like now" |

At recall time the two brains' results are **merged** for the AI. The left brain supplies the fact ("we discussed a job change last Tuesday"), the right brain supplies the context ("it was a rainy evening, a bit down"), so the AI can continue naturally with both — e.g., "Did that job-change decision settle down?"
...
[Call ends]
  ↓
VoiceMem analyzes the call
  ├─ Left store: phone number·name·date·time-of-day·topic·appointments (structured facts)
  └─ Right store: emotion·tone·weather·context → semantic embedding (vector)
        ↓
[Next call begins]
  ↓
Both brains queried at once for the same phone number
  ├─ Left: exact list of facts
  └─ Right: past memories matching the current time-of-day/mood (top similarity)
        ↓
Results merged and handed to the AI → AI speaks with facts + mood
```

### Engine choice — Ollama (local) or OpenAI API

VoiceMem's two brains use AI for **embedding generation** (the right brain's similarity search) and **call analysis** (emotion·topic·summary extraction). You choose **one** engine to handle both.

| Engine | Embedding | Analysis | Notes |
|--------|-----------|----------|-------|
| **Ollama** (default·local) | `nomic-embed-text` (768-dim) | local LLM | Free·offline·private. Needs extra setup on WSL2 ([see below](#ollama-embedding-auto-setup-wsl2)) |
| **OpenAI API** (optional·cloud) | `text-embedding-3-small`, etc. | GPT models | No Ollama install, consistent quality. Billed by usage |

> **Use exactly one (pick one).** Switching the engine moves embedding and analysis over wholesale. Memories saved with Ollama and with OpenAI use different embedding dimensions/models and don't mix, so **decide the engine up front and keep it consistent.** Switching later can degrade right-brain (similarity) recall of older memories (left-brain fact lookup is unaffected).

#### Enabling the OpenAI API key mode

Set the engine to OpenAI in `twilio-bot/ai_config.py`.

```python
# twilio-bot/ai_config.py
VOICEMEM_ENABLED = True             # use VoiceMem dual-brain (False to disable)
VOICEMEM_ENGINE = "openai"          # "ollama" (default) or "openai"
VOICEMEM_OPENAI_EMBED_MODEL = "text-embedding-3-small"   # right-brain embedding
VOICEMEM_OPENAI_CHAT_MODEL  = "gpt-4o-mini"              # call analysis (emotion·topic·summary)
```

Put the key in `.env` or Docker Secrets (Secrets recommended so the key doesn't end up in code/images).

```bash
# Option A) add to .env
echo "OPENAI_API_KEY=sk-..." >> ~/OpenWebUI/.env

# Option B) Docker Secrets (recommended)
echo -n "sk-..." > ~/OpenWebUI/secrets/openai_api_key
chmod 600 ~/OpenWebUI/secrets/openai_api_key
```
...
> With the OpenAI engine, **no Ollama install or WSL2 binding is needed.** Embedding and analysis both run on OpenAI, so recall quality stays consistent regardless of Ollama connectivity issues (dummy-vector warnings, etc.). In exchange, each call's store/recall makes OpenAI API calls billed by usage — check your usage regularly.

> **To disable VoiceMem entirely**, set `VOICEMEM_ENABLED = False`. Call-memory store/recall is then off, while the rest of the call·calendar·Telegram features keep working.

---

## 📍 Location Check (NAVER + Kakao + Google)
...
**When the call's main purpose is the location check** ("ask ○○ where they are")

```
[call answered]
  ↓
"Hello, ○○. I'm calling on behalf of the administrator.
 The administrator asked me to check your current location. Could you tell me where you are?
 Say yes if you agree, or no if you'd rather not."   ← greeting, then straight to consent
  ↓
[person says "yes"] → "Please tell me where you are right now"
   ...
   Try NAVER Geocoding → fall back to Kakao Local search if not found
      ↓  coordinates resolved → Google hybrid satellite image (NAVER map if no Google key)
   📨 Map image + address sent to the admin's Telegram
      ↓
   "Got it. I'll pass this on. Have a good day." → call ends
[person says "no"] → skip location check
```

**If it's a check-in call that also asked for location**, the bot keeps chatting after getting the location.

- Location calls **always start with a greeting** (so the admin's command text doesn't become the first thing said), then go straight to the consent question — no "how are you doing?" small talk is appended.
...
- **Plain check-in calls** (no location wording, e.g. "call ○○ to check in") are unaffected and chat naturally as before — no consent question appears.
...
- The map image uses Google hybrid satellite (falls back to NAVER Static Map if no Google key).
...
### Map image — Google satellite (optional)

Coordinates are resolved by NAVER/Kakao, and the **final map image** can be a Google hybrid satellite photo (satellite + place labels). Add a Google Maps key and the map sent to Telegram becomes a satellite image.

- **With a Google key** → Google hybrid satellite (building rooftops, real terrain + road/place labels)
- **Without / on failure** → automatic fallback to a NAVER map (safety net)

> hybrid overlays place and road names on the satellite image, making it easy to tell where the spot is. For a pure satellite image (no labels), change `"maptype": "hybrid"` to `"satellite"` in the code.

**Getting a Google Maps API key:**
1. Sign in to [Google Cloud Console](https://console.cloud.google.com) → select/create a project
2. Enable **Maps Static API** + link a **billing account** ($200 free monthly credit)
3. Create an **API key** (`AIza…`) → (recommended) restrict it to Maps Static API

> ⚠️ A Google key is tied to billing, so a leak can incur charges. Restrict the key and set a usage/budget alert.
...
**3) Google Maps (optional · for satellite map)**
1. Sign in to [Google Cloud Console](https://console.cloud.google.com) → select/create a project
2. Enable **Maps Static API** + link a **billing account** ($200 free monthly credit)
3. Create an **API key** (`AIza…`) → (recommended) restrict to Maps Static API
   - With it, maps arrive as Google hybrid satellite; without it, as a NAVER map. (See [Map image — Google satellite](#map-image--google-satellite-optional))

### Entering keys — during install or later

During install, answer `y` at "Enable NAVER Maps location check?" and enter the NAVER Client ID·Secret, then (optionally) the Kakao REST API key and (optionally) the Google Maps API key, in order. Keys are stored safely as **Docker Secrets** files.
...
echo -n "YOUR_GOOGLE_MAPS_API_KEY" >  secrets/google_maps_api_key
...
> `NAVER_MAPS_CLIENT_ID` lives in `.env` (env var); the Secret, Kakao key, and Google key are managed as `secrets/` files. The code reads env var → secrets file in order, so keys load safely from file even when a gunicorn worker doesn't inherit the environment.
...
## 🛰️ GPS Location Share (elder care)

Unlike Location Check (NAVER/Kakao/Google), which uses the location **spoken during a call**, this 14th tool sends the person's **live GPS coordinates** once they **tap [Allow] to consent**. It suits elder-care needs where an accurate real-time location matters — e.g. checking on a parent living alone.

> ⚠️ **Why a link:** a plain voice call cannot reach the other handset's GPS sensor (a physical limit of all phone systems). So this feature uses an **SMS link + browser location permission**. Coordinates are sent only if the person opens the link and allows location access, so **consent is always required**.

### Flow

```
① Admin: "check ○○'s GPS location"  (or specify a number)
   ↓
② The system sends the person a call + an SMS together
   Call: "The administrator wants to check your location. Please tap the link in the text."
   SMS:  "Location share: https://<domain>/gps-share/<token>"
   ↓
③ The person opens the link → a page:
   "This location will be sent once to the administrator" notice + [📍 Send my location] button
   ↓
④ Tap the button → browser asks "Allow location?" → [Allow]
   ↓
⑤ Live GPS → admin's Telegram (map image) + admin's SMS (coords + map link) ✅
```

For the person it's just: answer the call → tap the SMS link → tap "Send my location" → tap "Allow".

### Usage

Ask in natural language from the OpenWebUI chat or an admin call.

- `"check ○○'s GPS location"` → looks up the number from contacts and sends the link
- `"send a location-share link to 010-1234-5678"` → sends by number directly

Once sent, it replies: "Sent a location-share link to ○○ (call + SMS). If they consent within 30 minutes, their location will be delivered."

### Security (5 layers)

| Layer | What it does |
|-------|--------------|
| Random token | Link path is an unguessable long random string |
| One-time | Once a location is sent, that link expires immediately (no reuse) |
| Time limit | Auto-expires after 30 min by default (if unused) |
| HTTPS | Transport encryption (when the domain is https) |
| Target-number binding | Each link is tied to a specific person's number, so the admin knows **whose** location it is and links never get mixed up |

> The default expiry (30 min) can be tuned via `GPS_SHARE_TTL_SEC` in the code.

### Legal & ethical notes

The feature is designed so location is sent **only when the person taps [Allow]** on the link, in line with consent-based location use. Please observe the following. (General guidance, not legal advice — consult a professional before real-world use.)

- **Clear notice & consent**: the person must know what is sent and to whom before tapping. The share page states this.
- **Capacity**: if the person's capacity to consent is impaired (e.g. severe dementia), their consent alone may not suffice; check guardianship/legal-representative requirements.
- **No covert tracking**: obtaining location without the person's knowledge or by deception is unlawful. Always inform and obtain consent.
- **Purpose limitation**: use only for elder care / safety checks; don't share with third parties or repurpose.

### Applying to an existing install

To add just the GPS feature without a full reinstall, run the provided `patch-gps-14th-tool.sh` in the install folder (backup → code insert → rebuild, all automatic).

```bash
cd ~/OpenWebUI
bash patch-gps-14th-tool.sh
```

> The backend (call, SMS, GPS, Telegram) is applied by this patch. The OpenWebUI tool registration happens automatically on a full reinstall; if you only patched, you can add the `gps_location_share` tool manually from the admin screen.

Verify (direct call):

```bash
curl -s -X POST http://localhost:5000/gps-request \
  -H "X-API-Secret: $(cat secrets/api_secret)" \
  -H "Content-Type: application/json" \
  -d '{"name":"John"}'
```

The person gets a call + SMS; once they tap [Allow] on the link, the location arrives at the admin's Telegram and phone.

---

## Ollama Embedding Auto-setup (WSL2)

Memory Anchor and contact semantic search use **Ollama embeddings** (`nomic-embed-text`, 768-dim) by default. The installer handles the following **automatically**:

> **You can use OpenAI instead of local Ollama.** If you switch the VoiceMem dual-brain engine to the OpenAI API, you can skip the Ollama setup below (embeddings and analysis run in the cloud). See [VoiceMem Dual-Brain — Engine choice](#-voicemem-dual-brain-leftright-memory).
...
- 14 tools registered (including calendar, live whisper, voice engine settings, memory anchor, NAVER location check, GPS location share)
...
├── start-openwebui-VoiceMem-integrated.sh                  # main install (customer/personal choice · SERVICE_MODE switch)
├── setup-browser-agent-calendar-naver-search-api-only.sh   # browser agent + calendar
...
A. No. A plain voice call cannot access the other handset's GPS sensor (a physical limit of all phone systems). Instead, the **GPS Location Share** tool sends an SMS link, and the person taps [Allow] in the browser to send live GPS (consent required). → [GPS Location Share](#️-gps-location-share-elder-care)

**Q. Does GPS Location Share work without the person's consent?**
A. No — and it isn't designed to. Location is sent only when the person opens the link and taps [Allow]. Covert or forced tracking is unlawful; use this on a **consent basis** only, e.g. for elder care.

**Q. I want to change the GPS link's expiry.**
A. Default is 30 minutes. Change `GPS_SHARE_TTL_SEC` (seconds) in `twilio-bot/twilio_bot.py` and rebuild the bot. For security, don't set it too long.

**Q. The location report has coordinates but no map image.**
A. It's usually a permissions issue on the map-image folder (`twilio-bot/data/reports`) or a failed map API call. The latest script fixes folder ownership for the bot user at install time. Reinstall or rebuild with `docker compose up -d --build twilio-bot`.

**Q. I want the map as a satellite image.**
A. Add a Google Maps API key and the map arrives as a Google hybrid satellite photo (satellite + labels). Enter it during install after enabling the NAVER location feature, or put the key in `secrets/google_maps_api_key` and rebuild. Without a Google key, a NAVER map is used. → [Map image — Google satellite](#map-image--google-satellite-optional)

**Q. It's a satellite image but looks like a plain map.**
A. The default hybrid overlays road/place labels on the satellite image. In dense urban areas the labels and roads can make it look like a plain map, but rooftops and terrain are actual photography. For a label-free pure satellite image, change `"maptype": "hybrid"` to `"satellite"` in the code.

**Q. On a location call the AI starts with "where are you?" without any greeting.**
A. Previously the admin's command text became the first thing said. The latest version always starts a location call with a greeting ("Hello, ○○. I'm calling on behalf of the administrator…") and then asks for location consent. Reinstall or rebuild the bot.

**Q. I need to ask for location but the AI only makes small talk like "how are you?"**
A. The latest version drops the small-talk question on location calls and goes straight to the consent request after the greeting. (Plain check-in calls keep the conversational small talk.) Rebuild to apply.

**Q. It can't find station names like "Gangnam Station."**
A. NAVER Geocoding only resolves road/lot addresses, not station/business names (NAVER's Local search API was discontinued). Add a Kakao REST API key to also resolve station/business names. → [Location Check (NAVER + Kakao + Google)](#-location-check-naver--kakao--google)
