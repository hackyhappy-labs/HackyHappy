...
**② 아래 전체 내용을 복사해서 터미널에 붙여넣기**

```bash
cat > /tmp/register_tool_v3.py << 'PYEOF'
...
class Tools:
    def call_me(self) -> str:
        """관리자한테 전화를 걸어줍니다.
        사용 예: "나한테 전화해줘", "내 폰으로 전화해줘"
        """
        try:
            r = requests.post(f"{TOOL_SERVER}/tools/call-me", json={}, timeout=15)
            if r.status_code == 200:
                return r.json().get("message", "관리자님께 전화를 걸었습니다. 잠시 후 전화가 올 겁니다!")
            return f"전화 걸기 실패 (오류코드: {r.status_code})"
        except Exception as e:
            return f"오류: 서버 연결 실패 - {e}"

    def call_contact(self, name: str, mission: str = "안부 확인") -> str:
        """저장된 연락처에게 AI가 대신 안부전화를 걸어줍니다.
        통화 완료 후 관리자에게 SMS와 전화로 결과를 보고합니다.
        30초 안에 안받으면 SMS로 보고합니다.
        사용 예: "김철수한테 안부전화 해줘", "엄마한테 전화해서 안부 확인해줘"
        Args:
            name: 연락처에 저장된 이름 (예: 김철수, 엄마, 아빠)
            mission: 전화 목적 (예: 안부 확인, 모임 참석 여부 확인, 약 먹었는지 확인)
        """
        try:
            r = requests.post(f"{TOOL_SERVER}/tools/call-contact",
                            json={"name": name, "mission": mission}, timeout=15)
            if r.status_code == 200:
                return r.json().get("message", f"{name}님께 안부전화를 걸었습니다!")
            data = r.json()
            if "error" in data:
                return f"{name}님 연락처가 없습니다. 먼저 저장해 주세요. 예: '{name} 010-xxxx-xxxx 저장해줘'"
            return f"전화 걸기 실패: {r.text}"
        except Exception as e:
            return f"오류: 서버 연결 실패 - {e}"

    def get_contacts(self) -> str:
        """저장된 연락처 목록을 전체 조회합니다.
        사용 예: "저장된 연락처 알려줘", "연락처 목록 보여줘", "누구 저장돼있어?"
        """
        try:
            r = requests.get(f"{TOOL_SERVER}/tools/contacts", timeout=15)
            data = r.json()
            contacts = data.get("contacts", {})
            if not contacts:
                return "저장된 연락처가 없습니다. '이름 010-xxxx-xxxx 저장해줘' 라고 말씀해 주세요."
    ...
    def save_contact(self, name: str, number: str) -> str:
        """연락처를 저장합니다. 서버 재시작 후에도 영구적으로 유지됩니다.
        한국 번호는 010-xxxx-xxxx 형식으로 입력하면 자동 변환됩니다.
        사용 예: "김철수 010-1234-5678 저장해줘", "엄마 번호 010-9999-8888 등록해줘"
        Args:
            name: 저장할 이름 (예: 김철수, 엄마, 아빠, 친구)
            number: 전화번호 (예: 010-1234-5678 또는 +821012345678)
        """
        try:
            r = requests.post(f"{TOOL_SERVER}/tools/contacts/add",
                            json={"name": name, "number": number}, timeout=15)
            if r.status_code == 200:
                return r.json().get("message", f"{name}님 번호를 저장했습니다!")
            return f"저장 실패 (오류코드: {r.status_code})"
        except Exception as e:
            return f"오류: 서버 연결 실패 - {e}"

    def delete_contact(self, name: str) -> str:
        """연락처를 삭제합니다.
        사용 예: "김철수 연락처 삭제해줘", "엄마 번호 지워줘"
        Args:
            name: 삭제할 이름 (예: 김철수, 엄마)
        """
        try:
            r = requests.post(f"{TOOL_SERVER}/tools/contacts/delete",
                            json={"name": name, "number": ""}, timeout=15)
            if r.status_code == 200:
                return r.json().get("message", f"{name}님 연락처를 삭제했습니다!")
            return f"삭제 실패: {name}님 연락처를 찾을 수 없습니다."
        except Exception as e:
            return f"오류: 서버 연결 실패 - {e}"
'''

payload = {
    "id": "phone_assistant_v2",
    "name": "전화 어시스턴트",
    "description": "전화 걸기, 안부전화, 연락처 저장/삭제/조회",
    "content": tool_code,
    "meta": {
...
try:
    ...
    if "already registered" in body:
        print("⚠️  이미 등록됨 → 아래 명령으로 삭제 후 재등록:")
        print("    curl -s -X POST http://localhost:3000/api/v1/tools/phone_assistant_v2/delete \\")
        print("      -H \"Authorization: Bearer $OW_JWT\"")
    else:
        print("❌ 실패:", body[:200])
except Exception as e:
    print("❌ 오류:", str(e))
...
python3 /tmp/register_tool_v3.py "$OW_JWT"
```
