...
CAL_METHOD = '''    async def get_today_schedule(self, __user__: dict = {}) -> str:
...
info "신규 Tool: get_today_schedule (밸브에 OpenWebUI API 키 입력 필요)"
...
OLD_BROWSE = '''    async def browse(self, task, __event_emitter__=None):
        """Open a URL and perform a task. Do NOT use for weather/prices/stocks - use dedicated functions.
        :param task: URL + instruction
        """'''
NEW_BROWSE = '''    async def browse(self, task, __event_emitter__=None, __user__={}):
        """Open a URL and perform a task. Do NOT use for weather/prices/stocks - use dedicated functions.
        :param task: URL + instruction
        """
...
OLD_UM = '''    async def update_memory(self, info, __event_emitter__=None):
        """사용자 정보 저장.
        :param info: 기억할 정보
        """
        if not self.valves.ENABLE_MEMORY: return "메모리 비활성화"'''
NEW_UM = '''    async def update_memory(self, info, __event_emitter__=None, __user__={}):
        """사용자 정보 저장.
        :param info: 기억할 정보
        """
        if not self._set_user(__user__):
...
OLD_MAB = '''    async def multi_agent_browse(self, task, __event_emitter__=None, __user__={}):'''
