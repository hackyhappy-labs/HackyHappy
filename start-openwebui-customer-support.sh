...
def _create_calendar_event(title, start_epoch, end_epoch=None, all_day=False, location="", description=""):
...
def process_admin_command(speech, caller, bargein_note=""):
    ...
    if any(n in speech for n in _cal_add_nouns) and any(v in speech for v in _cal_add_verbs):
        ...
        ok, err = _create_calendar_event(title, start_epoch, all_day=all_day)
        if ok:
            ...
            return f"{when_txt}에 '{title}' 일정을 등록했습니다."
        return err
...
class Tools:
    ...
    def create_event(self, title: str, date: str, time: str = "", location: str = "", __user__: dict = {}) -> str:
        """캘린더에 새 일정을 등록합니다. '12월 25일 3시에 회의 등록해줘' 같은 요청에 사용.
        title: 일정 제목, date: YYYY-MM-DD, time: HH:MM(24시간, 없으면 종일), location: 장소(선택)."""
        deny = self._check_admin(__user__)
        if deny:
            return deny
        key = self._resolve_key()
        ...
        if not all_day:
            ...
            when_txt += " (종일)"
        return f"✅ 일정을 등록했습니다: {payload['title']} — {when_txt}"
'''
