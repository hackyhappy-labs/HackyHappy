...
############################################
read_secret() {
  # 민감 정보 입력: 타이핑 시 보이고, Enter 후 마스킹 처리
  local prompt="$1"
  local timeout="${2:-120}"
  local value=""
  read -t "$timeout" -p "$prompt" value || true
  value=$(echo "$value" | xargs)
  if [ -n "$value" ]; then
    local masked=$(echo "$value" | sed 's/./*/g')
    echo -ne "\r${prompt}${masked}          \n"
  else
    echo ""
  fi
  echo "$value"
...
echo "└────────────────────────────────────────────┘"
read -t 120 -p "🔑 Groq API Key 입력 (120초 내 Enter=건너뜀): " GROQ_API_KEY || true
GROQ_API_KEY=$(echo "$GROQ_API_KEY" | xargs)
if [ -n "$GROQ_API_KEY" ]; then
  echo -ne "\r🔑 Groq API Key 입력 (120초 내 Enter=건너뜀): $(echo "$GROQ_API_KEY" | sed 's/./*/g')          \n"
else
  echo ""
fi
if [ -n "$GROQ_API_KEY" ]; then
...
if [ -n "$TWILIO_ACCOUNT_SID" ]; then
  echo -ne "\r📞 Twilio Account SID 입력 (120초 내 Enter=건너뜀): $(echo "$TWILIO_ACCOUNT_SID" | sed 's/./*/g')          \n"
else
  echo ""
fi

read -t 120 -p "🔑 Twilio Auth Token 입력 (120초 내 Enter=건너뜀): " TWILIO_AUTH_TOKEN || true
TWILIO_AUTH_TOKEN=$(echo "$TWILIO_AUTH_TOKEN" | xargs)
if [ -n "$TWILIO_AUTH_TOKEN" ]; then
  echo -ne "\r🔑 Twilio Auth Token 입력 (120초 내 Enter=건너뜀): $(echo "$TWILIO_AUTH_TOKEN" | sed 's/./*/g')          \n"
else
  echo ""
fi
...
read -t 120 -p "🌐 서버 도메인 입력 ex) https://vulva.sex (120초 내 Enter=건너뜀): " SERVER_DOMAIN || true
...
OW_EMAIL=${OW_EMAIL:-"admin@vulva.sex"}

read -t 120 -p "   🔒 관리자 비밀번호 입력 (120초 내 Enter=자동생성): " OW_PASSWORD || true
OW_PASSWORD=$(echo "$OW_PASSWORD" | xargs)
if [ -n "$OW_PASSWORD" ]; then
  echo -ne "\r   🔒 관리자 비밀번호 입력 (120초 내 Enter=자동생성): $(echo "$OW_PASSWORD" | sed 's/./*/g')          \n"
else
  echo ""
...
read -t 120 -p "   🔢 관리자 PIN 6자리 설정 (120초 내 입력 필수): " ADMIN_PIN || true
ADMIN_PIN=$(echo "$ADMIN_PIN" | xargs)
if [ -n "$ADMIN_PIN" ]; then
  echo -ne "\r   🔢 관리자 PIN 6자리 설정 (120초 내 입력 필수): ******          \n"
fi

# PIN 보안 강화: 기본값/취약한 PIN 사용 금지
WEAK_PINS="123456 000000 111111 222222 333333 444444 555555 666666 777777 888888 999999 123123 654321 112233"
while true; do
  if [ -z "$ADMIN_PIN" ]; then
    echo "   ⚠️  PIN은 필수입니다. 6자리 숫자를 입력해주세요."
    read -t 120 -p "   🔢 관리자 PIN 6자리 설정: " ADMIN_PIN || true
    ADMIN_PIN=$(echo "$ADMIN_PIN" | xargs)
    continue
  fi
  if [ ${#ADMIN_PIN} -ne 6 ] || ! echo "$ADMIN_PIN" | grep -qE '^[0-9]{6}$'; then
    echo "   ⚠️  PIN은 정확히 6자리 숫자여야 합니다."
    read -t 120 -p "   🔢 관리자 PIN 6자리 설정: " ADMIN_PIN || true
    ADMIN_PIN=$(echo "$ADMIN_PIN" | xargs)
    continue
  fi
  ...
  for wp in $WEAK_PINS; do
    [ "$ADMIN_PIN" = "$wp" ] && IS_WEAK=true && break
  done
  if [ "$IS_WEAK" = true ]; then
    echo "   🚫 취약한 PIN은 사용할 수 없습니다. 다른 PIN을 입력해주세요."
    read -t 120 -p "   🔢 관리자 PIN 6자리 설정: " ADMIN_PIN || true
    ADMIN_PIN=$(echo "$ADMIN_PIN" | xargs)
    continue
  fi
...
def voice():
    ...
    if is_admin(caller):
        ...
        gather = Gather(input="speech", speechModel="phone_call", enhanced="true", action=f"{SERVER_DOMAIN}/respond",
...
def verify_pin():
    ...
    if hmac.compare_digest(digits, ADMIN_PIN):
        reset_pin_failure(caller)
        print(f"✅ PIN 인증 성공: {caller}")
        gather = Gather(input="speech", speechModel="phone_call", enhanced="true", action=f"{SERVER_DOMAIN}/respond",
                        language=tts_lang, timeout=TIMEOUT_INBOUND, speechTimeout="auto")
        gather.say(get_msg("PIN_SUCCESS", lang), voice=tts_voice, language=tts_lang)
        response.append(gather)
    else:
        fail_count = record_pin_failure(caller)
        remain = PIN_MAX_FAIL - fail_count
        print(f"❌ PIN 실패: {caller} ({fail_count}회)")

        if remain <= 0:
            response.say(get_msg("PIN_LOCKED", lang), voice=tts_voice, language=tts_lang)
            response.hangup()
        else:
            gather = Gather(input="dtmf", action=f"{SERVER_DOMAIN}/verify-pin",
                            numDigits="6", timeout=10)
            gather.say(get_msg("PIN_FAIL", lang).format(remain=remain), voice=tts_voice, language=tts_lang)
            response.append(gather)
            response.hangup()
...
def voice_out_simple():
    ...
    tts_lang = get_twilio_lang(lang)
    tts_voice = get_tts_voice(lang)
    gather = Gather(input="speech", speechModel="phone_call", enhanced="true", action=f"{SERVER_DOMAIN}/respond-admin",
                    language=tts_lang, timeout=TIMEOUT_INBOUND, speechTimeout="auto")
    gather.say(get_msg("GREETING_ADMIN", lang), voice=tts_voice, language=tts_lang)
    response.append(gather)
...
질문:"""
    ...
    try:
        ...
        speech_to = "auto"
    gather = Gather(input="speech", speechModel="phone_call", enhanced="true", action=action_url,
...
import os, uuid, time, csv, requests as http_requests
...
def list_media(folder: str = ""):
    ...
    if not target_dir.startswith(MEDIA_DIR):
...
def upload_page(request: Request):
    ...
    # 🔒 보안: 로컬 IP + Docker 내부 네트워크만 허용
    client_ip = request.client.host if request.client else ""
    is_local = client_ip in ("127.0.0.1", "::1", "localhost") or client_ip.startswith("172.") or client_ip.startswith("10.")
    if not is_local:
        return HTMLResponse("Access denied", status_code=403)
...
PYEOF
  ...
  if echo "$TOOL_RESULT" | grep -q "SUCCESS\|SKIP\|UPDATED"; then
